...
function get_goods_gys_price($goods_id, $attr_id)
{
    $final_price = $GLOBALS['db']->getOne('SELECT shop_price FROM ' . $GLOBALS['ecs']->table('goods') . " WHERE goods_id=".$goods_id);
    $where = "goods_id=".intval($goods_id)." AND ";
    $where .= db_create_in($attr_id, 'goods_attr_id');
    $sql = 'SELECT SUM(attr_gys_price) AS attr_gys_price FROM ' . $GLOBALS['ecs']->table('goods_attr') . " WHERE $where";
    $res = $GLOBALS['db']->getOne($sql);
    return $final_price + $res;
}

function get_order_gys_price($order_id)
{
    if(!$order_id){
        return 0.00;
    }
    $sql = 'SELECT SUM(gys_price*goods_number) AS gys_price FROM ' . $GLOBALS['ecs']->table('order_goods') . " WHERE order_id=".intval($order_id);
    $gys_price = $GLOBALS['db']->getOne($sql);
    if($gys_price>0){
        return $gys_price;
    }
    $sql = "SELECT goods_id,goods_attr_id,goods_number FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE order_id=".intval($order_id);
    $goods = $GLOBALS['db']->getAll($sql);
    $gys_price = 0.00;
    foreach ($goods as $k=>$v) {
        $gys_amount = get_goods_gys_price($v['goods_id'], $v['goods_attr_id']);
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $order['gys_price'] = price_format(get_order_gys_price($order_id));
    }
...
function order_due_field($alias = '')
{
    return order_amount_field($alias) .
            ...
            " - {$alias}bonus - {$alias}discount -{$alias}coupon_money";
}
 ...
 * @return array
 */
function gys_out_account($order_id, $text) {
    $log_num = $GLOBALS['db']->getOne("SELECT COUNT(id) FROM ".$GLOBALS['ecs']->table('gys_earn_log')." WHERE order_id=".intval($order_id)." AND amount>0");
    if($log_num>0){
        $row = $GLOBALS['db']->getRow("SELECT gys_id,SUM(amount) AS amount FROM ".$GLOBALS['ecs']->table('gys_earn_log')." WHERE order_id=".intval($order_id));
        if($row['amount']>0){
            $sql = "INSERT INTO ".$GLOBALS['ecs']->table('gys_earn_log')." VALUES('','$row[gys_id]','$order_id','-$row[amount]','".time()."','$text',1,1,2)";
            $agency_id = $GLOBALS['db']->getOne("SELECT agency_id FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id='$order_id'");
            if(!check_gys_account_by_agency_id($agency_id)){
                return array(
                    'error'=>1,
                    'info'=>'供应商账户异常,id'.$agency_id
                );
            }
            $GLOBALS['db']->query("SET AUTOCOMMIT=0");
            $res_inert_log = $GLOBALS['db']->query($sql);
            $gys_account = $GLOBALS['db']->getOne("SELECT gys_account FROM ".$GLOBALS['ecs']->table('admin_user')." WHERE agency_id='$agency_id'");
            $new_gys_account = (intval($gys_account*100)-intval($row['amount']*100))/100;
            $sql_update_gys_account = "UPDATE ".$GLOBALS['ecs']->table('admin_user')." SET gys_account='$new_gys_account' WHERE agency_id='$agency_id'";
            $res_update_account = $GLOBALS['db']->query($sql_update_gys_account);
            if($res_inert_log && $res_update_account){
                $GLOBALS['db']->query('COMMIT');
            }else{
                $GLOBALS['db']->query('ROLLBACK');
                file_put_contents('trans_log.txt', 'insert:'.$res_inert_log."\r\n".'update:'.$res_update_account, FILE_APPEND);
            }
 ...
 * @return array
 */
function check_gys_shipping_and_pay_first($order_id)
{
    $agency = getAgencyByOrderId($order_id);
    if (!$agency) {
        return array(
            'error' => 1,
            'info' => '没找到供应商信息'
    ...
    if (!check_gys_account_by_agency_id($agency['agency_id'])) {
        return array(
            'error' => 1,
            'info' => '供应商账户异常,请联系供应商管理员核对供应商信息'
        );
    }
    if ($agency['agency_pay_type'] == 0) {
        return array(
            'error' => 1,
            'info' => '供应商结算方式为：顾客下单付款后结算首期款'
        );
    }

    $firstMoneyHasPaid = $GLOBALS['db']->getOne("SELECT COUNT(id) FROM " . $GLOBALS['ecs']->table('gys_earn_log') . " WHERE order_id='$order_id' AND is_paid='1' AND in_out='0' AND f_step='0'");
    if ($firstMoneyHasPaid > 0) {
        return array(
            'error' => 1,
            'info' => '供应商首期款已结算'
        );
    ...
    $gys_price = get_order_gys_price($order_id);
    ...
    $data = array(
        ...
        'amount' => $inMoney,
        'add_time' => time(),
        'process_type' => '首期款' . $agency['agency_first_percent'] . '%',
        'is_paid' => 1
    );

    $GLOBALS['db']->query('SET AUTOCOMMIT=0');
    $gys = $GLOBALS['db']->getRow("SELECT user_id,gys_account FROM " . $GLOBALS['ecs']->table('admin_user') . " WHERE agency_id='".$agency['agency_id']."'");
 ...
 * @return mixed
 */
function getAgencyByOrderId($order_id) {
//    $this->table = 'order_info';
//    $agency_id = $this->field('agency_id', "order_id='$order_id'");
    $agency_id = $GLOBALS['db']->getOne('SELECT agency_id FROM '.$GLOBALS['ecs']->table('order_info')." WHERE order_id='$order_id'");
//    $this->table = 'agency';
//    $agency = $this->find("agency_id='$agency_id'", '*');
    $agency = $GLOBALS['db']->getRow("SELECT * FROM ".$GLOBALS['ecs']->table('agency')." WHERE agency_id='$agency_id'");
    return $agency;
}
 ...
 * @return bool
 */
function check_gys_account_by_agency_id($agency_id) {
    $num = $GLOBALS['db']->getOne("SELECT COUNT('user_id') FROM ".$GLOBALS['ecs']->table('admin_user')." WHERE agency_id='$agency_id'");
    return $num==1;
}
 ...
 * @param $order_id
 */
function received_and_pay_end($order_id){
...
//    $this->table = 'gys_earn_log';
    //该订单已入账首期款信息
    $firstMoney = $GLOBALS['db']->getRow("SELECT id,amount,add_time,is_paid FROM ".$GLOBALS['ecs']->table('gys_earn_log')." WHERE gys_id='$agency[agency_id]' AND order_id='$order_id' AND f_step='0'");
    $secMoney = $GLOBALS['db']->getRow("SELECT id,amount,add_time,is_paid FROM ".$GLOBALS['ecs']->table('gys_earn_log')." WHERE gys_id='$agency[agency_id]' AND order_id='$order_id' AND f_step='1'");
    ...
    if($firstMoney['amount']){
        ...
        $data = array(
            ...
            'amount'=> $inMoney,
            'add_time'=>time(),
            'process_type'=> '管理员确认收货,尾款'.(100-$agency['agency_first_percent']).'%',//入账类型(0:首期款,1尾款)
            'is_paid'=>1,
            'f_step'=>1
        );
//        file_put_contents('gysPayFlowError.log', date('Y-m-d H:i:s', time())."Error::firstMoney:".$firstMoney['amount'].'inMoney:'.$inMoney.' order_id:'.$order_id."\r\n", FILE_APPEND);
        ...
        if($agency['agency_pay_type'] == 0){
            ...
            $data = array(
                array(
                    ...
                    'amount'=> $firstInMoney,
                    'add_time'=>time(),
                    'process_type'=> '管理员确认收货,首期款'.$agency['agency_first_percent'].'%',//入账类型(0:首期款,1尾款)
                    'is_paid'=>1,
                    'f_step'=>0
                ),
                array(
    ...
    if($inMoney == 0){
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('gys_earn_log'), $data, 'INSERT');
...
//            $insert_id2 = $this->insert($data[1]);
            $insert_id1 = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('gys_earn_log'), $data[0], 'INSERT');
            $insert_id2 = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('gys_earn_log'), $data[1], 'INSERT');
