 ...
 * 改变余额
 */
function changeSurplus(val, coupon_id)
{
  ...
  var coupon_id = coupon_id || 0;
  $.get('index.php?m=default&c=flow&a=change_surplus', {surplus:val,coupon_id:coupon_id}, function(data){
      document.getElementById('ECS_SURPLUS').value = val;
...
//     // $.get('index.php?m=default&c=flow&a=choose_coupon&coupon_id='+coupon_id,'',function(data){
//     //     console.log(data);
//     // },'json');
// })
function chooseCoupon(val)
{
    ...
    // Ajax.call('flow.php?step=choose_coupon', 'coupon_id=' + val, chooseCouponResponse, 'GET', 'JSON');
    $.get('index.php?m=default&c=flow&a=choose_coupon&coupon_id='+val,'',function(data){
        chooseCouponResponse(data);
    },'json');
}
/*选择优惠券回调*/
function chooseCouponResponse(obj)
{
    if (obj.error)
    {
        try
        {
            document.getElementById("ECS_COUPON_NOTICE").innerHTML = obj.error;
            document.getElementById('coupon_use').innerHTML = '请选择';
        }
        catch (ex) { }
    }
    else
    {
        try
        {
            document.getElementById("ECS_COUPON_NOTICE").innerHTML = '';
        }
        catch (ex) { }
        couponChooseedResponse(obj.content);
    }
}
/* *
 * 回调函数
 */
function couponChooseedResponse(result)
{
    if (result.error)
    {
        alert(result.error);
