...
class sign extends PluginWechatController
{
    ...
    public function show($fromusername, $info)
    {
        ...
        $articles = array('type'=>'text', 'content'=>$config['sign_fail_value']?$config['sign_fail_value']:'签到失败');
        if (isset($config['point_status']) && $config['point_status'] == 1) {
            ...
            if ($num < $config['point_num']) {
                ...
                $articles['content'] = $config['sign_success_value']?$config['sign_success_value']:'签到成功，果豆已入袋。';
            } else {
                $articles['content'] = $config['sign_already_value']?$config['sign_already_value']:'签到次数已用完';
            }
        } else {
            $articles['content'] = $config['sign_disabled_value']?$config['sign_disabled_value']:'未启用签到送积分';
        }
        return $articles;
    }
