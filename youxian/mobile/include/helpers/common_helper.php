...
function send_sms_by_order_id_after_paid($order_id, $order_sn){
    $global = getInstance();

    $agency_id = $global->db->query("SELECT agency_id FROM ".$global->ecs->table('order_info')." WHERE order_id='".$order_id."'");
    $gys_info = $global->db->query("SELECT * FROM ".$global->ecs->table('agency')." WHERE agency_id='".$agency_id[0]['agency_id']."' LIMIT 1");
    $gys_info = $gys_info[0];
    if($gys_info['agency_phone']){
        require_once BASE_PATH . 'libraries/ClSms.class.php';
        $clapi = new ClSms();
        $content = "【讯罗优选】收到新的订单,订单编号".$order_sn.",请于48小时内发货。";
        ...
        $global->db->autoExecute($global->ecs->table('sms_log'), $sms_data, 'INSERT');
//        $global->db->table('sms_log')->data($sms_data)->insert();
    }else{
