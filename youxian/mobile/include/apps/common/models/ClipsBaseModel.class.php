...
class ClipsBaseModel extends BaseModel {
    ...
    public function insert_user_account($surplus, $amount) {
        ...
        $data['admin_user'] = $surplus['admin_note'];
        $data['amount'] = $amount;
    ...
    public function my_coupon($user_id){
	    $now = time();
//	    $sql = "SELECT COUNT(id) AS num FROM ".$this->pre."user_coupon WHERE user_id='$user_id' AND use_time='0' AND ";
	    $sql = "SELECT COUNT(uc.id) AS num FROM ".$this->pre."user_coupon AS uc LEFT JOIN ".$this->pre."coupon AS c ON uc.coupon_id=c.id WHERE uc.user_id='".$user_id."' AND uc.use_time='0' AND c.use_end_date>$now";
	    $res = $this->query($sql);
	    return $res[0]['num'];
    }
	/**
...
/*	   $where['user_id'] = $user_id;
     ...
     * 获取已付款未发货订单的数量
     */
    public function not_fahuo($user_id) {
        /*	   $where['user_id'] = $user_id;
               $where['pay_status'] = 0;
               $count = $this->model->table('order_info')->where($where)->count();*/

        $where = 'and pay_status = ' . PS_PAYED . ' and shipping_status=0 and order_status not in(' . OS_CANCELED . ','. OS_INVALID .','. OS_RETURNED .')';
        $sql = "SELECT count(*) as num FROM " . $this->pre . "order_info WHERE user_id = '$user_id' " . $where ;
        $res = $this->row($sql);
        return $res['num'];
    }
     ...
     * @return mixed
     */
    public function update_dls_deposit($dls_id, $deposit) {
        $this->table = 'dls';
        $data['dls_deposit'] = $deposit;
        $condition['dls_id'] = $dls_id;
     ...
     * @return bool
     */
    public function get_dls_from_pay_log($log_id) {
        $log_info = $this->row("SELECT * FROM ".$this->pre."pay_log WHERE log_id=$log_id");
        if($log_info['order_type'] == PAY_DLS_DEPOSIT){
            $this->table = 'user_account';
            $shop_user_id = $this->field('user_id', "id=".$log_info['order_id']);
            if(!$shop_user_id) return false;
            return $this->get_dls_by_user_id($shop_user_id);
        }else{
            return false;
        }
    }
    ...
    /**
     * 根据商城会员id，获取代理商信息,如果不是代理商返回false
     * @param $user_id
     * @return bool
     */
    public function get_dls_by_user_id($user_id) {
        $this->table = 'wechat_user';
        $openid = $this->field('openid', "ect_uid=".intval($user_id));
        if(!$openid) return false;
        $this->table = 'dls';
        $dls = $this->row("SELECT dls_id,username,phone,dls_deposit,cxs_id FROM ".$this->pre."dls WHERE openid='".$openid."'");
        return $dls;
    }

    public function insert_cxs_earn($pay_log, $dls) {
        $this->table = 'cxs_earn';
        $data = array(
            'addtime'=>time(),
            'pay_log_id'=>$pay_log['log_id'],
            'user_account_log_id'=>$pay_log['order_id'],
            'dls_id'=>$dls['dls_id'],
            'amount'=>C('cxs_earn_from_dls')
        );
        $cxs_earn = $this->insert($data);
        if($dls['cxs_id']){
            //更新承销商账户余额
            ...
            $old_account = $this->field('cxs_account', $condition);
            $account = 0.00+$old_account+$data['amount'];//获取承销商账户余额
            $data = array(
                'cxs_account'=>$account
            );
            $cxs_account_update = $this->update($condition, $data);
        }
        return $cxs_earn && $cxs_account_update;
    }


    public function insert_recharge_card_log($data){
        $this->table = 'recharge_card_log';
        return $this->insert($data);
    }

    public function insert_yx_activity_clothes($data){
        $this->table = 'activity_clothes';
        return $this->insert($data);
    }
}
