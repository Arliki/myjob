...
function fpc_debug($file, $content, $mode=FILE_APPEND, $path='data/debug_log/'){
    $content = is_array($content) ? implode(';',$content) : $content;
    $content = date('Y-m-d H:i:s', time())."::".$content.PHP_EOL;
    file_put_contents($path.$file, $content, $mode );
}
...
function arr_divide($arr, $col, $idx) {
    if(!is_array($arr) or !$col){
        return array();
    }
    $res = array_column($arr, $col, $idx);
    foreach ($res as $k=>$v) {
        if(!is_array($v)){
            $v = array(
                'gys_id'=>$k,
        ...
        foreach($arr as $key=>$val){
            if($val['goods_gys_id'] == $v['gys_id']){
                array_push($v['goods'], $val);
            }
        }
 ...
 * @checks  integer     $checks     活动添加的订单，时间已经转化过
 * @return  string
 */
function local_date($format, $time = NULL,$checks=0) {
    $timezone = isset($_SESSION['timezone']) ? $_SESSION['timezone'] : C('timezone');
...
function integral_format($integral) {
    return intval($integral).C('integral_name');
}
 ...
 * @param $name
 * @param $content
 * @param string $time 0只写入日期 -1 只写入内容 1 日期+内容
 */
function write_log($name,$content='',$time=0){
    switch ($time){
        case -1:
            $time = date('Y-m-d h:i:s', time());
            $fi = fopen(ROOT_PATH . "data/w_logs/" . $name, "a");
            flock($fi, LOCK_EX);
            fwrite($fi, "写入日期：" . $time . "\r\n");
            flock($fi, LOCK_UN);
            fclose($fi);
