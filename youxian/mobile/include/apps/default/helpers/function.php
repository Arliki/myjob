...
/*获取随机*/
function get_rand($bingo,$area,$leval){
    if ($bingo<=4){$bingo=4;}
    if ($area<=2){$area=2;}
    if ($leval==0){$leval=1;}
    $num['bingo']=floor(rand(1,$bingo)+$leval);
    $num['user']=floor(rand(1,pow(10,$area)));
    return $num;
}
function get_bingo_rand($bingo,$area,$leval){
    if ($area<=2){$area=2;}
    if ($leval==0){$leval=1;}
    $num['bingo']=floor(rand(1,$bingo)+$leval);
    $num['user']=floor(rand(1,$area));
    return $num;
}
/**
 * 图片兼容地址
...
//        $data['code'] = $code;
//        return __URL__ .'/respond.php?'.http_build_query($data);
    }
}
...
/**
 * @author mkvu52
 * @desc 将base64格式图片保存并返回路径
 * @param $dir
 * @param $data
 * @return string
 */
function save_base64_pic($dir, $data){
    if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $data, $result))
    {
        ...
        $new_file = $dir . uniqid() . '.'.$result[2];
        file_put_contents(ROOT_PATH.$new_file, base64_decode(str_replace($result[1], '', $data)));
        return $new_file;
    }
}