...
class IndexModel extends CommonModel {
    ...
    public function goods_list($type = 'best', $limit = 10, $start = 0) {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " . "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd, c.measure_unit AS goods_unit " . 'FROM ' . $this->pre . 'goods AS g ' . "LEFT JOIN " . $this->pre . "member_price AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
        $sql .= ' LEFT JOIN '.$this->pre . "category AS c ON g.cat_id=c.cat_id ";
    ...
    function get_index_nav_cate_goods(){
        $cids = $this->query("SELECT cat_id,cat_img FROM ".$this->pre."category"." WHERE parent_id='0' AND show_in_nav='1' ORDER BY sort_order ASC");
        foreach ($cids as $k=>$v){
            $v['goods'] = $this->get_top_cat_new_goods($v['cat_id']);
            $v['cat_img'] = '../'.$v['cat_img'];
            $v['cat_url'] = url('category/index',array('id'=>$v['cat_id']));
            $cids[$k] = $v;
        }
        return $cids;
    }

    function get_top_cat_new_goods($cid, $num=6) {
        $son_cids = $this->query("SELECT cat_id FROM ".$this->pre."category"." WHERE parent_id='$cid'");
        $son_cids = implode(',', array_column($son_cids, 'cat_id', null));
//        $sql = "SELECT g.goods_id,g.goods_img,g.shop_price,g.goods_name,g.is_hot,g.is_new,g.is_best,c.measure_unit AS unit FROM ".$this->pre."goods"." AS g LEFT JOIN ".$this->pre."category AS c ON g.cat_id=c.cat_id"." WHERE g.cat_id IN ($son_cids) AND g.is_delete=0 AND g.is_on_sale=1 AND g.goods_img!='' ORDER BY g.sort_order asc LIMIT 0, $num";
        $sql = "SELECT g.goods_id,g.goods_img,g.shop_price,g.goods_name,g.is_hot,g.is_new,g.is_best,(SELECT MIN(attr_price+0.0) FROM ".$this->pre."goods_attr WHERE goods_id=g.goods_id) AS attr_price,(SELECT attr_value FROM ".$this->pre."goods_attr WHERE goods_id=g.goods_id ORDER BY (attr_price+0.0) ASC LIMIT 1) AS attr_value FROM ".$this->pre."goods"." AS g"." WHERE ".($son_cids?"g.cat_id IN ($son_cids) AND":"")." g.is_delete=0 AND g.is_on_sale=1 AND g.goods_img!='' ORDER BY g.sort_order asc LIMIT 0, $num";
        $goods = $this->query($sql);
        foreach ($goods as $k=>$v){
            $v['goods_img'] = '../'.$v['goods_img'];
            $v['goods_url'] = url('goods/index',array('id'=>$v['goods_id']));
            $v['unit'] = $v['attr_value'];
            $v['shop_price'] = $v['shop_price']+$v['attr_price'];
            $goods[$k] = $v;
        }
        return $goods;
    }

    function get_hot_goods($num=4,$cid=0){
        $where = ' WHERE g.is_hot=1';
        if($cid>0){
            $where = " AND g.cat_id='$cid'";
        }
        $where .= " AND g.is_delete=0 AND g.goods_img!='' ";
        $sql = "SELECT g.goods_id,g.goods_img,g.shop_price,g.goods_name,g.is_hot,c.measure_unit AS unit FROM ".$this->pre."goods"." AS g LEFT JOIN ".$this->pre."category AS c ON g.cat_id=c.cat_id ".$where." ORDER BY g.add_time desc LIMIT 0, $num";
        $goods = $this->query($sql);
        foreach ($goods as $k=>$v){
            $v['goods_img'] = '../'.$v['goods_img'];
            $v['goods_url'] = url('goods/index',array('id'=>$v['goods_id']));
            $v['goods_brief'] = $v['goods_brief']==''?$v['goods_name']:$v['goods_brief'];
            $goods[$k] = $v;
        }
        return $goods;
    }
