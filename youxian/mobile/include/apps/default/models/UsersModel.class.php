...
class UsersModel extends BaseModel {
    ...
    function register($username, $password, $email, $other = array()) {
        ...
        if (!ECTouch::user()->add_user($username, $password, $email)) {
            ...
            //定义other合法的变量数组
            $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'parent_id', 'truename', 'serverno', 'idcard', 'user_rank', 'jjr_id', 'dls_id');
            ...
            if (isset($affiliate['on']) && $affiliate['on'] == 1) {
                ...
                if ($up_uid) {
                    ...
                    //推荐人赠送消费积分@author ActionNone
                    file_put_contents('tj_debug.log',date('Y-m-d H:i:s',time())."up_id:".$up_uid."\r\n", FILE_APPEND);
                    if(!empty($affiliate['config']['reg_pay_points'])){
                        file_put_contents('tj_debug.log',"pay_points:".$affiliate['config']['reg_pay_points']."\r\n", FILE_APPEND);
                        $res = model('ClipsBase')->log_account_change($up_uid, 0, 0, 0, $affiliate['config']['reg_pay_points'], '注册赠送消费积分');
                        file_put_contents('tj_debug.log',"res:".$res."\r\n", FILE_APPEND);
    ...
    function edit_profile($profile) {
        ...
        //'bank_account','bank_account_name','bank_name','bank_province','bank_city','bank_code'
        $cfg['bank_account'] = $profile['bank_account'];
        $cfg['bank_account_name'] = $profile['bank_account_name'];
        $cfg['bank_name'] = $profile['bank_name'];
        $cfg['bank_province'] = $profile['bank_province'];
        $cfg['bank_city'] = $profile['bank_city'];
        $cfg['bank_code'] = $profile['bank_code'];
    ...
    function get_profile($user_id) {
        ...
        $sql = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank," .
                " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, bank_account, bank_account_name,bank_name,bank_province,bank_city,bank_code " .
        ...
        //'bank_account_name','bank_name','bank_province','bank_city','bank_code
        $info['bank_account_name'] = $infos['bank_account_name'];
        $info['bank_name'] = $infos['bank_name'];
        $info['bank_province'] = $infos['bank_province'];
        $info['bank_city'] = $infos['bank_city'];
        $info['bank_code'] = $infos['bank_code'];
    ...
    function get_user_orders($user_id, $pay = 1, $num = 10, $start = 0) {
        ...
        $sql = "SELECT order_id, order_sn, shipping_id, order_status, shipping_status, pay_status, add_time,confirm_time,pay_time, " .
        ...
        foreach ($res as $key => $value) {
            ...
            //判断是否为活动添加的订单。
            if ($value['confirm_time']==$value['add_time'] && $value['confirm_time']==$value['pay_time']){
                $arr[] = array(
                    'order_id' => $value['order_id'],
                    'order_sn' => $value['order_sn'],
                    'img' => get_image_path(0, model('Order')->get_order_thumb($value['order_id'])),
                    'order_time' => local_date(C('time_format'), $value['add_time'],1),
    ...
    function affirm_received($order_id, $user_id = 0) {
        ...
        /* 修改订单发货状态为“确认收货” */ else {
            ...
            if ($this->query($sql)) {
                ...
                /*供应商入账处理*/
                model('Gys')->inAccountSec($order_id);
                /*供应商入账处理END*/
    ...
    function get_order_detail($order_id, $user_id = 0) {
        ...
        if ($order['pay_status'] == PS_UNPAYED && ($order['order_status'] == OS_UNCONFIRMED || $order['order_status'] == OS_CONFIRMED)) {
            ...
            if ($payment_info === false || substr($payment_info['pay_code'], 0 , 4) == 'pay_') {
                ...
                $order['pay_online'] = $pay_obj->get_code($od, $payment);
            }
        } else {
            $order['pay_online'] = '';
        }
    ...
    function get_consignee_by_address_id($address_id) {
        $sql = "SELECT * FROM ".$this->pre."user_address WHERE address_id=".intval($address_id);
        $res = $this->row($sql);
        return $res;
    }
    /**
    ...
    function order_fee($order, $goods, $consignee) {
        ...
        foreach ($goods AS $v) {
            foreach($v as $val){
                ...
                if($val['half_price']==1 && $val['goods_number']>1){
                    $_all=$val['goods_price'] * $val['goods_number'];
                    $total['goods_price'] += ($_all-floor($val['goods_number']/2)*$val['goods_price']*0.5);
                }else {
        ...
        if(!empty($order['coupon_id'])){
            $coupon = model('Order')->coupon_info($order['coupon_id']);
            $total['coupon_money'] = $coupon['coupon_money'];
            $total['coupon_formated'] = price_format($coupon['coupon_money'], false);
        }
        /*优惠券END*/
        /* 配送费用 */
        ...
        if ($order['extension_code'] == 'group_buy' && $group_buy['deposit'] > 0) {
            ...
            $total['auction_integral'] = $_SESSION['auction_integral'];
        } else {
        ...
        $order['coupon_money'] = $order['coupon_money'] > 0 ? $order['coupon_money'] : 0;
        if ($total['amount'] > 0) {
            if (isset($order['coupon_money']) && $order['coupon_money'] > $total['amount']) {
                $order['coupon_money'] = $total['amount'];
                $total['amount'] = 0;
            } else {
                $total['amount'] -= floatval($order['coupon_money']);
                $max_amount -= floatval($order['coupon_money']);
            }
        } else {
            $order['coupon_money'] = 0;
            $total['amount'] = 0;
        }
        $total['coupon_money'] = $order['coupon_money'];
        $total['coupon_formated'] = price_format($total['coupon_money'], false);
