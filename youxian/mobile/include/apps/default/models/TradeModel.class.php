...
class TradeModel extends CommonModel {
     ...
     * @param  $start
     */
    public function ticker( $limit = 10, $start = 0) {
     
		
        // 取出所有符合条件的商品数据，并将结果存入对应的推荐类型数组中
        $sql = 'SELECT g.goods_id, g.goods_name,  g.shop_price ,g.goods_sn, g.sell_start_date, g.sell_end_date, g.goods_state,t.high,t.low,t.close,t.open,t.volumn FROM ' . $this->pre . 'goods AS g ' . "LEFT JOIN " . $this->pre . "trade_shopdaily AS t ON t.shopno = g.goods_sn AND t.del_flag =0 and t.date='".date("Y-m-d",time())."'";
        $sql .= ' WHERE  g.is_delete = 0  ' ;
        $sql .= ' ORDER BY g.goods_id DESC limit ' . $start . ', ' . $limit;
