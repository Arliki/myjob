...
class OrderModel extends BaseModel {
    ...
    function available_payment_list($support_cod, $cod_fee = 0, $is_online = false) {
        ...
        foreach ($res as $key => $row) {
            ...
            if(is_file('./images/'.$row['pay_code'].'_icon.png')){
                $row['icon'] = './images/'.$row['pay_code'].'_icon.png';
            }
    ...
    function cart_goods($type = CART_GENERAL_GOODS) {
        $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number," .
                ...
                "goods_price * goods_number AS subtotal, (SELECT gys_id FROM ".$this->pre."goods WHERE goods_id=c.goods_id) AS gys_id,(SELECT half_price FROM ".$this->pre."goods WHERE goods_id=c.goods_id) AS half_price,(SELECT goods_thumb FROM ".$this->pre."goods WHERE goods_id=c.goods_id) AS goods_thumb " .
                "FROM " . $this->pre .
                "cart AS c WHERE session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type'";
        ...
        foreach ($arr as $key => $value) {
            ...
            if($value['half_price']==1 && $value['goods_number']>1){
                $_all=$value['goods_price'] * $value['goods_number'];
                $value['subtotal'] = ($_all-floor($value['goods_number']/2)*$value['goods_price']*0.5);
            }
     ...
     * 购物车商品按照供应商分组
     */
    function cart_goods_group_by_gys($type = CART_GENERAL_GOODS){
        $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            "market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, " .
            "goods_price * goods_number AS subtotal, (SELECT gys_id FROM ".$this->pre."goods WHERE goods_id=c.goods_id) AS gys_id, (SELECT half_price FROM ".$this->pre."goods WHERE goods_id=c.goods_id) AS half_price " .
            "FROM " . $this->pre .
            "cart AS c WHERE session_id = '" . SESS_ID . "' " .
            "AND rec_type = '$type'";
        ...
        $ret = array();
        foreach($arr as $k=>$v){
            $ret[$v['gys_id']][$v['goods_id']][] = $v;
            if($v['half_price']==1 && $v['goods_number']>1){
                $_all=$v['goods_price'] * $v['goods_number'];
                $v['subtotal'] = ($_all-floor($v['goods_number']/2)*$v['goods_price']*0.5);
            }
//            if(isset($ret[$v['gys_id']])){
//                if(!isset($ret[$v['gys_id']][$v['goods_id']])){
//                    $ret[$v['gys_id']][$v['goods_id']] = $v;
//                }
//            }else{
//                $ret[$v['gys_id']][$v['goods_id']] = $v;
//            }
    ...
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0) {
        ...
        /*供应商供应价格*/
        $spec_gys_price = model('Goods')->spec_gys_price($spec);
        $goods_gys_price = $goods['org_price'] + $spec_gys_price;
        $gys_id = model('Goods')->get_gys_id($goods_id);
        /* 初始化要插入购物车的基本件数据 */
        $parent = array(
...
//            $attr = str_replace('[0]', '', $attr);
            $attr = preg_replace('/\[[\d\.]+\]/', '', $attr); //隐藏规格加价
        }
    ...
    function coupon_info($coupon_id){
        $sql = "SELECT uc.*,c.coupon_money FROM ".$this->pre."user_coupon AS uc LEFT JOIN ".$this->pre."coupon AS c ON uc.coupon_id=c.id WHERE uc.id='$coupon_id'";
        return $this->row($sql);
    }
    ...
    function get_cart_goods() {
        ...
        /* 循环、统计 */
        $sql = "SELECT c.*,g.half_price, IF(c.parent_id, c.parent_id, c.goods_id) AS pid " .
            " FROM " . $this->pre . "cart as c" ." LEFT JOIN ".$this->pre . "goods as g on c.goods_id=g.goods_id".
        ...
        foreach ($res as $row) {
            ...
            if($row['half_price']==1 && $row['goods_number']>1){
                $_all=$row['goods_price'] * $row['goods_number'];
                $total['goods_price'] += ($_all-floor($row['goods_number']/2)*$row['goods_price']*0.5);
            }else {
    ...
    function get_consignee($user_id) {
        if (isset($_SESSION['flow_consignee'])) {
            ...
            if ($user_id > 0) {
                /* 取默认地址 */
                $sql = "SELECT ua.*,(SELECT region_name FROM ".$this->pre."region WHERE region_id=ua.province) AS province_name,(SELECT region_name FROM ".$this->pre."region WHERE region_id=ua.city) AS city_name,(SELECT region_name FROM ".$this->pre."region WHERE region_id=ua.district) AS district_name" .
