...
class GoodsModel extends BaseModel {
    ...
    function get_goods_info($goods_id) {
        ...
        $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
                ...
                "IFNULL(gys.agency_name, '') AS gys_name " .
                ...
                " LEFT JOIN " . $this->pre . "agency AS gys ON gys.agency_id=g.gys_id " .
    ...
    function get_goods_cheapest_attr($goods_id){
//        $sql = "SELECT goods_attr_id,goods_id, attr_id, attr_value, MIN(attr_price) " .
//            "FROM " . $this->pre . "goods_attr " .
//            "WHERE goods_id='$goods_id' GROUP BY attr_id ";
        $sql = "SELECT t.attr_value, a.attr_name FROM (SELECT * FROM ".$this->pre."goods_attr WHERE goods_id='$goods_id' ORDER BY attr_price+0 ASC) AS t LEFT JOIN ".$this->pre."attribute AS a ON t.attr_id=a.attr_id GROUP BY t.attr_id";
    ...
    function spec_gys_price($spec) {
        if (!empty($spec)) {
            ...
            $sql = 'SELECT SUM(attr_gys_price) AS attr_gys_price FROM ' . $this->pre . "goods_attr WHERE $where";
            $res = $this->row($sql);
            $price = floatval($res['attr_gys_price']);
        } else {
            $price = 0;
        }
        return $price;
    ...
    function get_gys_id($goods_id) {
        return $this->field('gys_id', "goods_id='$goods_id'");
    }
}
