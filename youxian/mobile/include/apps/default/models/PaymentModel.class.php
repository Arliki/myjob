...
//                        }
                ...
                } elseif (($pay_log['order_type'] == PAY_SURPLUS) OR ($pay_log['order_type'] == PAY_DLS_DEPOSIT)) {
                    ...
                    if ($pay_log['order_type'] == PAY_DLS_DEPOSIT){
                        //TODO 代理商缴纳诚意金 支付成功后续业务
                        $dls = model('ClipsBase')->get_dls_from_pay_log($pay_log['log_id']);
                        //更新代理商 缴纳诚意金状态
                        $a = model('ClipsBase')->update_dls_deposit($dls['dls_id'], 1);
                        //插入承销商返现记录并更新承销商余额
                        $b = model('ClipsBase')->insert_cxs_earn($pay_log, $dls);
                    }
                }
            } else {
                /* 取得已发货的虚拟商品信息 */
