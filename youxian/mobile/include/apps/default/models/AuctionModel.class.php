...
class AuctionModel extends BaseModel {
     ...
     * @return  array
     */
    function auction_list($size, $page, $sort, $order, $where) {
        $auction_list = array();
//        $auction_list['finished'] = $auction_list['finished'] = array();
        ...
        $sql = "SELECT a.*,t.act_banner ,t.sales_count ,t.click_num ,  IFNULL(g.goods_thumb, '') AS goods_thumb " .
                ...
                $where.
//                "AND a.start_time <= '$now' AND a.end_time >= '$now' AND a.is_finished < 2 ".
                "ORDER BY $sort $order LIMIT $start ,$size ";
        $res = $this->query($sql);
        ...
        foreach ($res as $row) {
            ...
            $auction['formated_start_price'] = integral_format($auction['start_price']);
            $auction['formated_end_price'] = integral_format($auction['end_price']);
            $auction['formated_deposit'] = integral_format($auction['deposit']);
...
//        $auction_list = @array_merge($auction_list['under_way'], $auction_list['finished']);
    ...
    function auction_info($act_id, $config = false) {
        ...
        if ($config == true) {
            $auction['start_time'] = local_date('m/d H:i', $auction['start_time']);
            $auction['end_time'] = local_date('m/d H:i', $auction['end_time']);
        } else {
            $auction['start_time'] = local_date(C('time_format'), $auction['start_time']);
            $auction['end_time'] = local_date(C('time_format'), $auction['end_time']);
        }
        ...
        $auction['formated_start_price'] = integral_format($auction['start_price']);
        $auction['formated_end_price'] = integral_format($auction['end_price']);
        $auction['formated_amplitude'] = integral_format($auction['amplitude']);
        $auction['formated_deposit'] = integral_format($auction['deposit']);
        ...
        $auction['formated_current_price'] = integral_format($auction['current_price'], false);
    ...
    function auction_log($act_id) {
        ...
        foreach ($res as $row) {
            $row['bid_time'] = local_date(C('time_format'), $row['bid_time']);
            $row['formated_bid_price'] = integral_format($row['bid_price'], false);
            $log[] = $row;
        }
        return $log;
    }
