...
class UserController extends CommonController {
    ...
    public function __construct() {
        ...
        $this->assign('integralName', C('integral_name'));
        assign_template();
...
//            'bonus'=> $bonus,
    ...
    public function profile() {
        ...
        if (IS_POST) {
            ...
            $profile = array(
                ...
                'bank_name'=>$bank[1],
                'bank_province'=>I('post.bank_province'),
                'bank_city'=>I('post.bank_city'),
                'bank_code'=>$bank[0],
    ...
    public function recharge_card(){
        if(IS_POST){
            ...
            if(!$card_num || !$password){
                show_message('请输入卡号和密码');
            }
            if(!preg_match('/^[ABCDEFWZ]YX\d{6}$/', $card_num)){
                show_message('卡号错误,请重新输入');
            }
            if(!preg_match('/^[a-zA-Z0-9]{10}$/', $password)){
                show_message('密码错误,请重新输入');
            }
            $sql = "SELECT * FROM ".$this->model->pre."recharge_card WHERE card_num='$card_num'";
            $card = M()->getRow($sql);
            if(!$card){
                show_message('卡号输入有误,请重新输入');
            }
            $sql_use = "SELECT COUNT(id) AS use_times FROM ".$this->model->pre."recharge_card_log WHERE user_id=".$this->user_id." AND card_value='".$card['price']."' AND success=1";
            $has_used_same_value_card = M()->getRow($sql_use);

            if($card['multiuse']<1 && $has_used_same_value_card['use_times']>0){
                show_message('此面值充值卡每用户限用一张');
            }

            $user_ip = get_client_ip();

            if(isset($_SESSION['recharge_card_denied_uid']) && $this->user_id == $_SESSION['recharge_card_denied_uid']){
                show_message('您已多次尝试失败,请联系客服');
            }
            ...
            if(strtoupper($card['password']) === $password){
                if($card['canuse']<1){
                    $log_data['success'] = 0;
                    $log_data['msg'] = '卡未激活，有用户尝试充值';
                    model('ClipsBase')->insert_recharge_card_log($log_data);
                    show_message('该卡未激活');
                }
                if($card['usetime'] > 0){
                    $log_data['success'] = 0;
                    $log_data['msg'] = '卡已使用，有用户尝试充值';
                    model('ClipsBase')->insert_recharge_card_log($log_data);
                    show_message('该卡已使用,充值失败');
                }
            }else{
                $log_data['success'] = 0;
                $log_data['msg'] = '卡号密码错误';
                model('ClipsBase')->insert_recharge_card_log($log_data);
                $fail_time = isset($_SESSION['recharge_card_fail_time']) ? $_SESSION['recharge_card_fail_time'] : 0;
                $_SESSION['recharge_card_fail_time'] = $fail_time + 1;
                if($_SESSION['recharge_card_fail_time'] >=10){
                    $_SESSION['recharge_card_denied_uid'] = $this->user_id;
                    M()->query("UPDATE " .M()->pre . "recharge_card SET watchout= 1 WHERE id = '".$card['id']."'");
                    show_message('您已连续10次输错密码,请确认卡号密码或联系客服');
                }
                show_message('卡号或密码错误,请核对后重新输入');
            }
            //更新充值卡使用时间
            M()->query("UPDATE " .M()->pre . "recharge_card SET usetime= '".time()."' WHERE id = '".$card['id']."'");
            //记录充值日志
            $log_data['success'] = 1;
            $log_data['msg'] = '充值成功';
            model('ClipsBase')->insert_recharge_card_log($log_data);
            //更新用户账户余额
            $change_desc = '充值卡充值,卡号'.$card['card_num'];
            model('ClipsBase')->log_account_change($this->user_id, $card['price'], 0,0,0,$change_desc);
            show_message('充值成功', '', url('User/account_detail'));
        }

        $this->assign('title', '充值卡充值');
        $this->display('user_recharge_card.dwt');
    }
    /**
     *  删除会员余额 
     */
    ...
    public function act_account()
    {
        ...
        else
        {
            ...
            $payment_info['pay_button'] = $pay_obj->get_code($od, $payment);
    ...
    public function order_detail() {
		...
		if($order['coupon_money']){
            $order['formated_coupon'] = price_format($order['coupon_money'], false);
        }
    ...
    public function edit_surplus() {
        ...
        $has_virtual_goods_sql = "SELECT * FROM " . $this->model->pre . "order_goods WHERE order_id='$order[order_id]' AND is_real = 0 AND extension_code = 'virtual_card'";
        $has_virtual_goods = $this->model->query($has_virtual_goods_sql);

        if ($has_virtual_goods) {
            /* 虚拟卡发货 */
    ...
    public function share(){
        $is_jjr = $_SESSION['user_rank']==6?true:false;
        if($is_jjr){
            $reg_user_num = self::$db->getOne("SELECT COUNT(user_id) FROM ".self::$ecs->table('users')." WHERE jjr_id='".$_SESSION['user_id']."' OR parent_id='".$_SESSION['user_id']."'");
            $tj_users = self::$db->getAll("SELECT FROM_UNIXTIME(u.reg_time,'%Y/%m/%d') AS addtime,wu.headimgurl,IFNULL(wu.nickname, u.user_name) FROM ".self::$ecs->table('users')." AS u LEFT JOIN ".self::$ecs->table('wechat_user')." AS wu ON wu.ect_uid=u.user_id WHERE u.jjr_id='".$_SESSION['user_id']."' OR parent_id='".$_SESSION['user_id']."'");
        }else{
            $reg_user_num = self::$db->getOne("SELECT COUNT(user_id) FROM ".self::$ecs->table('users')." WHERE parent_id='".$_SESSION['user_id']."'");
            $tj_users = self::$db->getAll("SELECT FROM_UNIXTIME(u.reg_time,'%Y/%m/%d') AS addtime,wu.headimgurl,wu.nickname FROM ".self::$ecs->table('users')." AS u LEFT JOIN ".self::$ecs->table('wechat_user')." AS wu ON wu.ect_uid=u.user_id WHERE u.parent_id='".$_SESSION['user_id']."'");
        }
        $this->assign('title', '我的推荐');
        $this->assign('tj_num', $reg_user_num);
        $this->assign('tj_users', $tj_users);
        $this->display('user_share.dwt');
    }
    /**
     * 分享推荐
     */
    public function share_backup() {
...
//        var_dump($share);
        ...
        $this->assign('qrcode_url', $this->sence_qrcode());
        $this->display('user_share.backup.dwt');
    }
    ...
    public function share_list() {
        ...
        switch ($level){
            case 1:
                $where = "u.parent_id='$this->user_id'";
                break;
            case 2:
                ...
                $res1 = $this->model->table('users')
                    ...
                    ->where($where)
                    ->select();
                $uid_l1 = implode(',',array_column($res1, 'user_id', 'user_id'));
                if($uid_l1){
                    $where = "u.parent_id in ($uid_l1)";
                }else{
                    $no_user = true;
                }
                break;
            case 3:
                ...
                if(!$uid_l1){
                    ...
                    $where = "parent_id in ($uid_l1)";
                    $res2 = $this->model->table('users')
                        ->field('user_id')
                        ->where($where)
                        ->select();
                    $uid_l2 = implode(',',array_column($res2, 'user_id', 'user_id'));
                    if(!$uid_l2){
                        $no_user = true;
                    }else{
                        $where = "u.parent_id in ($uid_l2)";
                    }
                }
        ...
        $this->assign('level', $level.'级下线');
        if($no_user===true){
            $this->assign('no_users', $no_user);
            $this->assign('count', 0);
            $this->display('user_share_list.dwt');
        }else{
            $sql = "SELECT u.user_id,u.user_name,wu.headimgurl,wu.nickname FROM ".self::ecs()->prefix."users AS u LEFT JOIN ".self::ecs()->prefix."wechat_user AS wu ON u.user_id=wu.ect_uid WHERE ".$where;
            $res = $this->model->query($sql);
            foreach($res as $k=>$v){
                if(!$v['headimgurl'] || !$v['nickname']){
                    $v['headimgurl'] = "__TPL__/images/member-photo-img2.jpeg";
                    $v['nickname'] = $v['user_name'];
                    $res[$k] = $v;
                }
            }
            $this->assign('count', count($res));
            $this->assign('my_users', $res);
            $this->display('user_share_list.dwt');
        }
...
//        $info = model('ClipsBase')->get_user_default($this->user_id);
        $userinfo = model('Users')->get_profile($this->user_id);
        if($userinfo['user_rank']==6){
            //经纪人
            $scene_id = 'J'.$this->user_id;
            $expire = 0;
        }else{
            $scene_id = $this->user_id;
            $expire = 2592000;
        }
        $qrcode = WechatController::rec_qrcode($userinfo['user_name'], $scene_id, $expire, '代理分销');
        if($qrcode){
...
//        return $qrcode;
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            if (0 == $enabled_sms) {
				...
				$other['truename'] = isset($_POST['truename']) ? in($_POST['truename']) : '';
				$other['idcard'] = isset($_POST['idcard']) ? in($_POST['idcard']) : '';
				$other['serverno'] = isset($_POST['serverno']) ? in($_POST['serverno']) : '';
			//	$other['mobile_phone'] = isset($_POST['mobile']) ? in($_POST['mobile']) : '';
                if (empty($username)) {
                    show_message(L('msg_mobile_blank'), L('register_back'), url('register'), 'error');
                }
				 if (!	$other['truename']) {
                    show_message("姓名不能为空", L('register_back'), url('register'), 'error');
                }
                if (!	$other['idcard']) {
                    show_message("身份证不能为空", L('register_back'), url('register'), 'error');
                }
                if (!	$other['serverno']) {
                    show_message("服务号不能为空", L('register_back'), url('register'), 'error');
                }
                ...
                // 验证手机号重复
                // $where['mobile_phone'] = $username;
                $user_id = $this->model->table('users')->field('user_id')->where('mobile_phone='.$username)->getOne();
                if ($user_id) {
                    show_message(L('msg_mobile_exists'), L('register_back'), url('register'), 'error');
                }
                 // 验证服务号重复
                // $where['mobile_phone'] = $username;
                $s_no = $this->model->table('users')->field('user_id')->where('msn='.$other['serverno'])->getOne();
                 if (!$s_no) {
                    show_message("服务号不存在", L('register_back'), url('register'), 'error');
                }
            } else {
    ...
    private function check_login() {
        ...
        // 未登录处理
        if (empty($_SESSION['openid']) && empty($_SESSION['user_id']) && !in_array($this->action, $without)) {
