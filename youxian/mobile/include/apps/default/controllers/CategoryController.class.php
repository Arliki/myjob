...
class CategoryController extends CommonController {
    ...
    public function async_list() {
        ...
        echo json_encode(array('list' => $goodslist));
...
//            $this->keywords .= "(goods_name LIKE '%$val%' OR goods_sn LIKE '%$val%' OR keywords LIKE '%$val%' )";
            $this->keywords .= "(goods_name LIKE '%$val%' OR goods_sn LIKE '%$val%' )";
    ...
    private function category_get_goods() {
        ...
        $sort = $this->sort == 'sales_volume' ? 'xl.sales_volume' : 'g.'.$this->sort;
        /* 获得商品列表 */
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, g.goods_number, " .'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img, xl.sales_volume, c.measure_unit AS unit, (SELECT MIN(attr_price+0.0) FROM '.$this->model->pre.'goods_attr WHERE goods_id=g.goods_id) AS attr_price,(SELECT attr_value FROM '.$this->model->pre.'goods_attr WHERE goods_id=g.goods_id ORDER BY (attr_price+0.0) ASC LIMIT 1) AS attr_value ' . 'FROM ' . $this->model->pre . 'goods AS g ' . ' LEFT JOIN ' . $this->model->pre . 'touch_goods AS xl ' . ' ON g.goods_id=xl.goods_id ' . ' LEFT JOIN ' . $this->model->pre . 'member_price AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' LEFT JOIN ecs_category AS c ON g.cat_id=c.cat_id "  . "WHERE $where $this->ext ORDER BY $sort $this->order LIMIT $start , $this->size";
        ...
        foreach ($res as $row) {
            ...
            if ($watermark_img != '') {
                $tmpArr['watermark_img'] = $watermark_img;
            }

            $tmpArr['goods_id'] = $row['goods_id'];
            if ($display == 'grid') {
                $tmpArr['goods_name'] = C('goods_name_length') > 0 ? sub_str($row['goods_name'], C('goods_name_length')) : $row['goods_name'];
            } else {
                $tmpArr['goods_name'] = $row['goods_name'];
            }
            $tmpArr['name'] = $row['goods_name'];
            $tmpArr['goods_brief'] = $row['goods_brief'];
            $tmpArr['goods_style_name'] = add_style($row['goods_name'], $row['goods_name_style']);
            $tmpArr['market_price'] = price_format($row['market_price']);
            $tmpArr['shop_price'] = price_format($row['shop_price']);
            $tmpArr['type'] = $row['goods_type'];
            $tmpArr['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
            $tmpArr['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb'], true);
            $tmpArr['goods_img'] = get_image_path($row['goods_id'], $row['goods_img']);
            $tmpArr['url'] = url('goods/index', array(
                'id' => $row['goods_id']
            ));
            $tmpArr['sales_count'] = $sales_volume;
            $tmpArr['sc'] = model('GoodsBase')->get_goods_collect($row['goods_id']);
            $tmpArr['mysc'] = 0;
            // 检查是否已经存在于用户的收藏夹
            if ($_SESSION['user_id']) {
                unset($where);
            ...
            $tmpArr['goods_sales'] = $this->get_goods_sales($row['goods_id']);
            $tmpArr['goods_number'] = $row['goods_number'];
            $tmpArr['promotion'] = model('GoodsBase')->get_promotion_show($row['goods_id']);
            $tmpArr['comment_count'] = model('Comment')->get_goods_comment($row['goods_id'], 0);  //商品总评论数量
            $tmpArr['favorable_count'] = model('Comment')->favorable_comment($row['goods_id'], 0);  //获得商品好评百分比

            $tmpArr['unit'] = $row['unit'];

            $tmpArr['unit'] = $row['attr_value'];
            $tmpArr['shop_price'] = price_format($row['shop_price']+$row['attr_price']);
