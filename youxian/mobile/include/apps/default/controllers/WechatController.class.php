    ...
    public function index()
    {
        ...
        if ($type == Wechat::MSGTYPE_TEXT) {
            ...
            if ('subscribe' == $wedata['Event']) {
                ...
                $fans_openid = $wedata['FromUserName'];
                $this->auto_login($fans_openid);
...
//            $keywords = $qrcode_fun;
//            $this->tjgz_msg($wedata['FromUserName'], $scene_id);
        }
        // 回复
        if (! empty($keywords)) {
        ...
        if(!empty($openid)){
            ...
            if (empty($rs)) {
                ...
                if(empty($ect_uid)){
                    ...
                    // 用户注册
                    $to_reg_user_data = array(
                        'user_rank'=>0,
                        'parent_id'=>0,
                        'jjr_id'=>0,
                        'dls_id'=>0
                    );
                    //TODO 根据不同场景值，自动获取要注册新用户的关联字段（经纪人id，代理商id，parent_id）
                    $scene_id = empty($scene_id) ? 0 : $scene_id;
                    file_put_contents('qrcode_debug.log','dls:'.$scene_id."\r\n",FILE_APPEND);
                    if(preg_match('/^D(\d)+$/', $scene_id)){
                        //场景为代理商二维码
                        $dls_id = mb_substr($scene_id, 1);
                        $dls_name = $this->model->table("dls")->field("username")->where(array('dls_id'=>$dls_id))->getOne();
                        if($dls_name){
                            $to_reg_user_data['user_rank'] = 6;//代理商发展的会员为经纪人
                            $to_reg_user_data['dls_id'] = $dls_id;

//                            $template .= "\r\n欢迎您成为经纪人,您的代理商是 ".$dls_name;
                            $template .= "\r\n欢迎您成为经纪人！";
                        }
                    }elseif (preg_match('/^J(\d)+$/', $scene_id)){
                        //场景为经纪人二维码
                        $jjr_id = mb_substr($scene_id, 1);
                        $jjr_info = $this->model->table("users")->field("user_id,dls_id,user_name")->where(array('user_id'=>$jjr_id))->find();
                        if($jjr_info){
                            $to_reg_user_data['jjr_id'] = $jjr_info['user_id'];
                            $to_reg_user_data['dls_id'] = $jjr_info['dls_id'];
                            $jjr_name = $this->model->table("wechat_user")->field("nickname")->where(array('ect_uid'=>$jjr_id))->getOne();
                            $jjr_name = empty($jjr_name)?$jjr_info['user_name']:$jjr_name;
                            $template .= "\r\n欢迎您成为注册会员,您的推荐人是 ".$jjr_name;
                        }
                    }else{
                        //场景为普通注册会员分享推荐
                        $parent_info = $this->model->table("users")->field('user_id,user_name')->where(array('user_id'=>$scene_id))->find();
                        $parent_id = empty($parent_info['user_id']) ? 0 : $parent_info['user_id'];
                        $to_reg_user_data['parent_id'] = $parent_id;
                        $parent_name = $this->model->table("wechat_user")->field("nickname")->where(array('ect_uid'=>$parent_id))->getOne();
                        $parent_name = empty($parent_name)?$parent_info['user_name']:$parent_name;
                        $template .= "\r\n欢迎您的加入,您的推荐人是 ".$parent_name;
                    }

                    $domain = get_top_domain();
                    if (model('Users')->register($username, $password, $username . '@' . $domain, $to_reg_user_data) !== false) {
                ...
                if(0 && !empty($template) || !empty($bonus_msg)){
                    // 微信端发送消息
                    $msg = array(
                        'touser' => $openid,
                        'msgtype' => 'text',
...
//                $data['subscribe'] = $info['subscribe'];
    ...
    {
//        file_put_contents('qrcode_debug.log','dls:'.$type."\r\n",FILE_APPEND);
    ...
    public function auto_login($openid){
        if(!$_SESSION['openid'] || !$_SESSION['user_id']){
            $user_id = $this->model->table('wechat_user')->field('ect_uid')->where("openid='$openid'")->find();
            $user_id = $user_id['ect_uid'];
            if(!$user_id){
                self::snsapi_userinfo();
            }else{
                $user_name = $this->model->table('users')->field('user_name')->where("user_id='$user_id'")->find();
    ...
    public function tjgz_msg($fromusername, $keywords)
    {
        $return = false;
        $rs = $this->model->table('wechat_extend')
            ->field('name, keywords, command, config')
            ->where('command = "tjgz" and enable = 1 and wechat_id = ' . $this->wechat_id)
            ->order('id asc')
            ->find();
     ...
     * @return [type]             [description]
     */
    static function rec_qrcode_backup($user_name = '', $user_id = 0, $expire_seconds = 0, $fun = ''){
        if(empty($user_id)){
            return false;
        }
        // 默认公众号信息
    ...
    static function rec_qrcode($user_name = '', $user_id = 0, $expire_seconds = 0, $fun = '', $type=0){
        if(empty($user_id)){
            return false;
        }
        // 默认公众号信息
        ...
        if (! empty($wxinfo) && $wxinfo['type'] == 2) {
            ...
            elseif($qrcode['id'] && empty($qrcode['qrcode_url'])){
                $ticket = $weObj->getQRCode($qrcode['scene_id'], $qrcode['type'], $qrcode['expire_seconds']);
                if (empty($ticket)) {
                    //$weObj->errCode, $weObj->errMsg
                    return false;
                }
                ...
                $data['scene_id'] = $user_id;
                $data['username'] = $user_name;
                $data['type'] = $type ? $type : (empty($expire_seconds) ? 1 : 0);
                $data['wechat_id'] = $wxinfo['id'];
                $data['status'] = 1;
                //生成二维码
                $ticket = $weObj->getQRCode($data['scene_id'], $data['type'], $expire_seconds);
                if (empty($ticket)) {
                    //$weObj->errCode, $weObj->errMsg
                    return false;
                }
