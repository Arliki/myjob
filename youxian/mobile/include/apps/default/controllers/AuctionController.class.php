...
class AuctionController extends CommonController {
    ...
    public function index() {
        ...
        $this->assign('finished', I('get.finished',0,'intval'));
    ...
    public function asynclist(){
        ...
        $now = gmtime();
        if(I('get.finished',0,'intval')==1){
            //已结束
            $where = " AND (a.end_time<'$now' OR a.is_finished>=1) ";
        }else{
            //进行中
            $where = " AND a.start_time <= '$now' AND a.end_time >= '$now' AND a.is_finished < 2 ";
        }
        $list = model('Auction')->auction_list($this->size, $this->page, $this->sort, $this->order, $where);
    ...
    public function info() {
        ...
        /* 取得拍卖活动信息 */
        $auction = model('Auction')->auction_info($id, true);
        if (empty($auction)) {
            $this->redirect(url('Auction/index'));
            exit;
        }
    ...
    public function bid() {
        ...
        if ($auction['deposit'] > 0) {
            /* 可用资金够吗 */
            if ($user['pay_points'] < $auction['deposit']) {
                show_message('可用'.C('integral_name').'不够交保证金，此拍卖品需要缴纳保证金'.$auction['deposit'].C('integral_name'), '', '', 'error');
            }

            /* 如果不是第一个出价，解冻上一个用户的保证金 */
            if ($auction['bid_user_count'] > 0) {
                model('ClipsBase')->log_account_change($auction['last_bid']['bid_user'], 0, 0, 0, $auction['deposit'], sprintf(L('au_unfreeze_deposit'), $auction['act_name']));
            }

            /* 冻结当前用户的保证金 */
            model('ClipsBase')->log_account_change($user_id, 0, 0, 0, (-1) * $auction['deposit'], sprintf(L('au_freeze_deposit'), $auction['act_name']));
        }
    ...
    public function buy() {
        ...
        $cart = array(
            ...
            'goods_price' => 0,//$auction['last_bid']['bid_price'],
            'goods_number' => 1,
...
//            'rec_type' => CART_EXCHANGE_GOODS,
        ...
        $_SESSION['extension_code'] = 'auction_goods';
        $_SESSION['extension_id'] = $id;
        $_SESSION['auction_integral'] = $auction['last_bid']['bid_price'];
        /* 进入收货人页面 */
