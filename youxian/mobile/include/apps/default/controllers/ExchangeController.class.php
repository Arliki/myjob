...
class ExchangeController extends CommonController {
    ...
    public function index() {
        ...
        $goods_list = model('Exchange')->exchange_get_goods($this->children, $this->integral_min, $this->integral_max, $this->ext, $this->size, $this->page, $this->sort, $this->order,$cid);
        $is_show=empty($cid)?$_SESSION['is_show']:$cid;
        $count = model('Exchange')->get_exchange_goods_count($this->children, $this->integral_min, $this->integral_max,$cid);
        $category=model('Exchange')->get_category();
    ...
    public function exchange_goods() {
        ...
        $min_goods_attr = model('Goods')->get_goods_cheapest_attr($goods['goods_id']);
        $this->assign('min_goods_attr', $min_goods_attr);
        $this->assign('goods_id', $goods_id);
        $this->assign('pictures', model('GoodsBase')->get_goods_gallery($goods_id));
        $this->assign('cfg', C('CFG'));

        $this->assign('wx_title', $goods['goods_name']);
        $this->assign('wx_url', __HOST__ . $_SERVER['REQUEST_URI']);
        $this->assign('wx_pic', __HOST__ .$goods['goods_thumb']);
        $this->assign('wx_desc', $goods['goods_name'].$goods['keywords']);
    ...
    public function buy() {
        ...
        if ($_SESSION['user_id'] <= 0) {
            //直接跳转
            $redirect_url = urlencode(url('exchange/exchange_goods',array('gid'=>$goods_id)));
            ecs_header("Location: " . url('user/login',array('referer'=>$redirect_url)) . "\n");
        ...
        if ((!empty($specs)) && ($product_info['product_number'] == 0) && (C('use_storage') == 1 && $goods['goods_number']==0)) {
            show_message(L('eg_error_number'), array(L('back_up_page')), array($back_act), 'error');
        }
