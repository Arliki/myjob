...
class FlowController extends CommonController {
    ...
    {
        parent::__construct();
        header("Access-Control-Allow-Origin:wx.cnxunluo.cn");
    }
    ...
    public function checkout() {
        ...
        } elseif ($flow_type == CART_AUCTION_GOODS){
            //TODO 拍卖
            $this->assign('is_auction_goods', 1);
        }else {
            // 正常购物流程 清空其他购物流程情况
        ...
        if ($count == 0) {
            show_message('购物车空空的，快去逛逛吧！', '去逛逛', url('Index/index'), 'warning');
        }
...
//        $order = array();
        ...
        /* 计算折扣 */
        if ($flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_AUCTION_GOODS) {
            $discount = model('Order')->compute_discount();
            $this->assign('discount', $discount ['discount']);
            $favour_name = empty($discount ['name']) ? '' : join(',', $discount ['name']);
            $this->assign('your_discount', sprintf(L('your_discount'), $favour_name, price_format($discount ['discount'])));
        ...
        //计算订单的费用
        $total = model('Users')->order_fee($order, array($cart_goods), $consignee);
        ...
        if ((isset($use_integral) || C('use_integral') == '1') && $_SESSION ['user_id'] > 0 && $user_info ['pay_points'] > 0 && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_AUCTION_GOODS)) {
            // 能使用积分
        ...
        if($flow_type == CART_EXCHANGE_GOODS){
            $need_integral = model('Flow')->exchange_goods_need_point($cart_goods[0]['goods_id']);
            $this->assign('exchange_need_integral', $need_integral); // 可用积分
            $this->assign('your_integral', $user_info ['pay_points']); // 用户积分
        }
        if($flow_type == CART_AUCTION_GOODS){
            $need_integral = (integer) $total['auction_integral'];
            $this->assign('auction_need_integral', $need_integral); // 可用积分
            $this->assign('your_integral', $user_info ['pay_points']); // 用户积分
        }
//        var_dump($need_integral);
        if($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_AUCTION_GOODS){
            /*优惠券 @author ActionNone*/
            $goods_total_price = $total['goods_price'];
            $now = time();
            $user_coupons = $this->model->query("SELECT uc.*,c.coupon_title,c.coupon_money,c.min_money,c.use_start_date,c.use_end_date FROM ".$this->model->pre."user_coupon AS uc LEFT JOIN ".$this->model->pre."coupon AS c ON uc.coupon_id=c.id WHERE uc.user_id='".$_SESSION['user_id']."' AND uc.disable='0' AND uc.use_time='0' AND c.min_money<=$goods_total_price AND c.use_start_date<$now AND c.use_end_date>$now ORDER BY c.coupon_money DESC");
            $this->assign('coupon_list', $user_coupons);
            /*优惠券 END*/
        }

        /* 如果使用红包，取得用户可以使用的红包及用户选择的红包 */
        $use_bonus = C('use_bonus');
        if ((!isset($use_bonus) || C('use_bonus') == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_AUCTION_GOODS)) {
            // 取得用户可用红包
    ...
    public function choose_consignee() {
        if(IS_AJAX){
            $address_id = intval($_POST['address_id']);
            if(!$address_id){
                echo json_encode(array('success'=>0,'info'=>'请选择收货地址'));die;
            }
            if (!$_SESSION ['user_id']) {
                echo json_encode(array('success'=>0,'info'=>'请登录','url'=>url('user/login',array('referer'=>url('flow/consignee_list')))));die;
            }
            $consignee = model('Users')->get_consignee_by_address_id($address_id);
            if(!$consignee){
                echo json_encode(array('success'=>0,'info'=>'没找到收货地址'));die;
            }
            if($consignee ['user_id'] != $_SESSION ['user_id']){
                echo json_encode(array('success'=>0,'info'=>'收货地址非法'));die;
            }
            model('Users')->save_consignee_default($address_id);
            /* 保存到session */
            $_SESSION ['flow_consignee'] = stripslashes_deep($consignee);
...
//            ))
        ...
        if (isset($_SESSION ['flow_type']) && intval($_SESSION ['flow_type']) != CART_GENERAL_GOODS) {
            $baseOrder ['extension_code'] = $_SESSION ['extension_code'];
            $baseOrder ['extension_id'] = $_SESSION ['extension_id'];
        } else {
            $baseOrder ['extension_code'] = '';
            $baseOrder ['extension_id'] = 0;
        }
        ...
        if ($user_id > 0) {
            ...
            $baseOrder ['surplus'] = min($baseOrder ['surplus'], $user_info ['user_money'] + $user_info ['credit_line']);
            if ($baseOrder ['surplus'] < 0) {
                $baseOrder ['surplus'] = 0;
            }
            //订单绑定经纪人和代理商
            $baseOrder['jjr_id'] = $user_info['jjr_id'];
            if($user_info['user_rank'] == 6){
                $baseOrder['jjr_id'] = $user_info['user_id'];
            }
            $baseOrder['dls_id'] = $user_info['dls_id'];
            // 查询用户有多少积分
            $flow_points = model('Flow')->flow_available_points(); // 该订单允许使用的积分
            $user_points = $user_info ['pay_points']; // 用户的积分总数

            $baseOrder ['integral'] = min($baseOrder ['integral'], $user_points, $flow_points);
            if ($baseOrder ['integral'] < 0) {
                $baseOrder ['integral'] = 0;
            }
        } else {
            $baseOrder ['surplus'] = 0;
            $baseOrder ['integral'] = 0;
        ...
        elseif(isset($_POST ['bonus_sn'])) {
            ...
            if (empty($bonus) || $bonus ['user_id'] > 0 || $bonus ['order_id'] > 0 || $bonus ['min_goods_amount'] > model('Order')->cart_amount(true, $flow_type) || $now > $bonus ['use_end_date']) {
                ...
                $baseOrder ['bonus_id'] = $bonus ['bonus_id'];
                $baseOrder ['bonus_sn'] = $bonus_sn;
            }
        }

        /*优惠券 @author ActionNone*/
        if($baseOrder['coupon_id']){
            if($user_id>0){
                $now = time();
                $coupon_info = $this->model->query("SELECT uc.*,c.coupon_title,c.coupon_money,c.min_money,c.use_start_date,c.use_end_date FROM ".$this->model->pre."user_coupon AS uc LEFT JOIN ".$this->model->pre."coupon AS c ON uc.coupon_id=c.id WHERE uc.id='".$baseOrder['coupon_id']."' AND uc.user_id='".$_SESSION['user_id']."' AND uc.disable='0' AND uc.use_time='0' AND c.use_start_date<$now AND c.use_end_date>$now;");
                if(!$coupon_info){
                    show_message('优惠券信息有误');
                }
                $coupon_info = $coupon_info[0];
                $baseOrder['coupon_money'] = $coupon_info['coupon_money'];
            }
        }

        /* 订单中的商品 */
        $cart_goods = model('Order')->cart_goods_group_by_gys($flow_type);
        ...
        $cart_total_price = M()->table('cart')->field('SUM(goods_price*goods_number)')->where("session_id='".SESS_ID."'")->getOne();

        $percent_of_cart_except_last = 0.00;
        $percent_of_cart_except_last = 0;
        $orders = array();
        $totals = array();
        $total_amount = 0.00;
        $log_ids = array();
        ...
        foreach($cart_goods as $k=>$v){
            ...
            if($v==end($cart_goods)){
                $percent_of_cart = 1 - $percent_of_cart_except_last;
            }else{
//                $order_total = array_sum(array_map(function($val){return $val['subtotal'];}, $v));
                foreach ($v as $val){
                    foreach($val as $val_goods){
                        $order_total += $val_goods['subtotal'];
                    }
                }

                $percent_of_cart = round($order_total / $cart_total_price, 2);
                $percent_of_cart_except_last += $percent_of_cart;
            }
            $order['surplus'] = $order['surplus'] - $total_use_surplus;
            $order['integral'] = $order['integral'] * $percent_of_cart;
            $order['coupon_money'] = $order['coupon_money'] * $percent_of_cart;
            ...
            /* 订单中的总额 */
            $total = model('Users')->order_fee($order, $v, $consignee);
            ...
            }elseif ($order['extension_code'] == 'auction_goods'){
                $order ['integral_money'] = 0;
                $order ['integral'] = $total ['auction_integral'];
            }
            $order ['from_ad'] = !empty($_SESSION ['from_ad']) ? $_SESSION ['from_ad'] : '0';
            $order ['referer'] = !empty($_SESSION ['referer']) ? addslashes($_SESSION ['referer']). 'Touch' : '商城';
...
//            file_put_contents('Duplicate.debug.log',date('Y-m-d H:i:s',time()).$order['order_sn'].':::ERR'.$error_no."\r\n",FILE_APPEND);
            ...
            if($_SESSION['isis']) {
                $sql = "INSERT INTO " . $this->model->pre . "order_goods( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, gys_price) " . " values  ('".$order ['order_id']."','" . $_SESSION['super'] . "','" . $_SESSION['goods_names'] . "', '" . $_SESSION['goods_sns'] . "',0,1,'" . $_SESSION['bid_price'] . "','" . $_SESSION['bid_price'] . "','抢购活动专属',1,'1',0,0,1,0)";
                file_put_contents('dddd.json',"user_id=".$order ['order_id'].";insert sql = ".$sql.PHP_EOL,FILE_APPEND);
            }else {
                /* 插入订单商品 */
                $condition_gys_str = '';
                if($flow_type==CART_GENERAL_GOODS){
                    $condition_gys_str = " AND gys_id='$k'";
                }
                $sql = "INSERT INTO " . $this->model->pre . "order_goods( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, gys_price) " . " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, gys_price " . " FROM " . $this->model->pre . "cart WHERE session_id = '" . SESS_ID . "' AND rec_type = '$flow_type' $condition_gys_str";
            }
            $this->model->query($sql);
            /* 修改拍卖活动状态 */
            if ($order ['extension_code'] == 'auction_goods') {
                $sql = "UPDATE " . $this->model->pre . "goods_activity SET is_finished='2' WHERE act_id=" . $order ['extension_id'];
                $this->model->query($sql);
            }
            /*更新优惠券状态 @author ActionNone*/
            if($order['coupon_id']>0 && $order['user_id']){
                $sql = "UPDATE " . $this->model->pre . "user_coupon SET use_time=".time().",order_id='$new_order_id' WHERE id=" . $order ['coupon_id'];
                $this->model->query($sql);
            }
            ...
            /*如果全部使用余额支付,分配供应商利润，并发送收到订单短信通知*/
            if($order ['order_amount'] == 0 && $flow_type==CART_GENERAL_GOODS){
                model('Gys')->inAccount($new_order_id);
                //供应商短信通知
                $agency_id = $this->model->query("SELECT agency_id FROM ".$this->model->pre."order_info WHERE order_id='".$new_order_id."'");
                ...
                if($gys_info['agency_phone']){
                    ...
                    $clapi = new ClSms();
                    $content = "【讯罗优选】收到新的订单,订单编号".$order['order_sn'].",请于48小时内发货。";
...
//                $this->assign('pay_desc',$order['pay_desc']);
            ...
            $order['total'] = $total;
            $orders[] = $order;
            $total_amount = $total_amount + floatval($order['order_amount']);
            $total_use_surplus = $total_use_surplus + floatval($order ['surplus']);
            $log_ids[] = $order ['log_id'];
            $titles[] = $order['order_sn'];
            $ids[] = $order['order_id'];
        }

        if ($total_amount > 0) {
            $payment = model('Order')->payment_info($orders[0]['pay_id']);
            include_once (ROOT_PATH . 'plugins/payment/' . $payment ['pay_code'] . '.php');
            $pay_obj = new $payment ['pay_code'] ();

            $od = array(
                'total_amount'=>$total_amount,
                'log_id'=>implode('|', $log_ids),
                'body'=>implode(';', $titles),
                'ids'=>implode('|', $ids)
            );
            $pay_online = $pay_obj->get_code($od, unserialize_config($payment ['pay_config']));
    ...
    public function change_surplus() {
        ...
        if ($user_info['user_money'] + $user_info['credit_line'] < $surplus) {
            ...
            if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
                ...
                if($coupon_id){
//                    $coupon = model('Order')->coupon_info($coupon_id);
...
//                    $order['coupon_money'] = $coupon['coupon_money'];
    ...
    public function consignee_list() {
        if(!$_SESSION['user_id']){
            $this->redirect(url('user/login',array('referer'=>urlencode(url('consignee_list')))));
        }
        if (IS_AJAX) {
            $start = $_POST ['last'];
            $limit = $_POST ['amount'];
            ...
            if ($consignee_list) {
                foreach ($consignee_list as $k => $v) {
                    ...
                    if($_SESSION['flow_consignee']){
                        $this->assign('default_address_id',$_SESSION['flow_consignee']['address_id']);
                    }
                    $this->assign('request_url',url('flow/choose_consignee'));
                    $this->assign('return_url',url('flow/checkout'));
                    $this->assign('check_text','使用');
                    $sayList [] = array(
                        'single_item' => ECTouch::view()->fetch('library/asynclist_info.lbi')
                    );
                }
