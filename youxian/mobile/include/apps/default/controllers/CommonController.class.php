...
class CommonController extends BaseController
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->ecshop_init();
        $third_login_controllers = array('Flow','User','Goods','Super','Dls','Index','Exchange','Games');
        if(in_array(CONTROLLER_NAME, $third_login_controllers) && !$_SESSION['user_id'] && !$_SESSION['openid']){
        ...
        if(in_array(CONTROLLER_NAME, array('Goods','Exchange')) && is_wechat_browser()){
...
//                $jsticket = $wechat->getJsTicket();
                $jsSign = $wechat->getJsSign(__HOST__ . $_SERVER['REQUEST_URI']);
        ...
        // 微信oauth处理a
//        if(!$_SESSION['user_id'] && !(CONTROLLER_NAME=='Wechat' || (ACTION_NAME=='third_login' && CONTROLLER_NAME=='User'))){
//            if(class_exists('WechatController')){
//                if (method_exists('WechatController', 'snsapi_userinfo')) {
//                    call_user_func(array('WechatController', 'snsapi_userinfo'));
//                }
...
//            exit('<h1 style="font-size: 5rem;text-align: center;margin-top: 40%;">'.$close_comment.'</h1>');
        ...
        if (! empty($_COOKIE['ECS']['user_id']) && ! empty($_COOKIE['ECS']['password'])) {
            ...
            if ($row) {
                ...
                //@author ActionNone 20171102 15:47
                if(!$_SESSION['openid']){
//                    $openid = $this->model->table('wechat_user')->field('openid')->where(array('ect_uid'=>$_SESSION['user_id']))->getCol();
                    $openid = M()->query("SELECT openid FROM ecs_wechat_user WHERE ect_uid='".$_SESSION['user_id']."' LIMIT 1");
                    $openid = $openid[0]['openid'];
                    if($openid){
                        $_SESSION['openid'] = $openid;
                    }else{
                        file_put_contents("showPayBtn.debug.log",date('Y-m-d H:i:s', time())."::CommonController没找到粉丝绑定信息\r\n",FILE_APPEND);
                    }
                    file_put_contents("showPayBtn.debug.log",date('Y-m-d H:i:s', time())."::CommonController---openid:$openid\r\n",FILE_APPEND);
                }
                model('Users')->update_user_info();
            } else {
                // 没有找到这个记录
