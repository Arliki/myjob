...
//            $nocestr = $wechat->generateNonceStr();
//            $wechat->checkAuth();
//            $jsticket = $wechat->getJsTicket();
////            var_dump($jsticket);var_dump($nocestr);
//            $jsSign = $wechat->getJsSign(__HOST__ . $_SERVER['REQUEST_URI'], 0, $nocestr, $config['appid']);
////            var_dump($jsSign);die;
//            $this->assign('appid', $jsSign['appid']);
//            $this->assign('timestamp', $jsSign['timestamp']);
//            $this->assign('noncestr', $jsSign['noncestr']);
//            $this->assign('signature', $jsSign['signature']);
//        }
    }
...
//        $this->assign('sales_count', $this->goods_id + 5800 + model('GoodsBase')->get_sales_count($this->goods_id));

        $sales_count = model('GoodsBase')->get_sales_count($this->goods_id);
        $sales_count = $sales_count + $goods['sale_count'];
        ...
        $this->assign('wx_title', $page_info['title']);
        $this->assign('wx_url', __HOST__ . $_SERVER['REQUEST_URI']);
        $this->assign('wx_pic', __HOST__ .$goods['goods_thumb']);
        $this->assign('wx_desc', $page_info['title']);
    ...
    public function price() {
            ...
            if ($this->goods_id == 0) {
                ...
                $goods = $this->model->table('goods')->field('goods_name , goods_number ,extension_code,half_price')->where($condition)->find();
...
// 			}
                ...
                if($goods['half_price']==1 && $number>1) {
                    $_all=$shop_price * $number;
                    $res ['result'] = price_format($_all-floor($number/2)*$shop_price*0.5);
                    $res['is_half'] = "第二件商品半价";
                }else {
