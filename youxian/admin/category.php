...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('cat_info',     array('is_show' => 0));
...
if ($_REQUEST['act'] == 'insert')
{
    /* 权限检查 */
    admin_priv('cat_manage');
    $cat_img_url = trim($_POST['cat_img_url']);
    if ((isset($_FILES['cat_img']['error']) && $_FILES['ad_img']['error'] == 0) || (!isset($_FILES['cat_img']['error']) && isset($_FILES['cat_img']['tmp_name'] ) &&$_FILES['cat_img']['tmp_name'] != 'none'))
    {
        $cat_img_url = $image->upload_image($_FILES['cat_img'], 'afficheimg');
    ...
    $cat['is_exchange']  = !empty($_POST['is_exchange'])  ? intval($_POST['is_exchange']): 0;
    if($cat['is_exchange']){
        $cat['parent_id']=0;
        $cat['is_show']=0;
    }
 ...
 * 检查是否兑换分类
 */
function check_exchange($cat_id){
    $sql1="select goods_id from ecs_exchange_goods where cat_id=$cat_id ";
    $old_id=$GLOBALS['db']->getAll($sql1);
    if($old_id){
        $str="";
        foreach ($old_id as $k=>$v){
            $str .=$v['goods_id'].",";
        }
        $str .="-1";
        $sql2="update ecs_exchange_goods set cat_id=190 where goods_id in ($str)";
