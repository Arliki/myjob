...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt,role_id,agency_id".
            " FROM " . $ecs->table('admin_user') .
            " WHERE user_name = '" . mysql_real_escape_string($_POST['username']). "' AND password = '" . md5(md5($_POST['password']).$ec_salt) . "'";
    }
    else
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt,role_id,agency_id".
            " FROM " . $ecs->table('admin_user') .
            " WHERE user_name = '" . mysql_real_escape_string($_POST['username']). "' AND password = '" . md5($_POST['password']) . "'";
    }
    $row = $db->getRow($sql);
    if ($row)
    {
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    if($role === 'gys'){
        $smarty->assign('ur_here',     '供应商账户列表');
        $smarty->assign('action_link', array('href'=>'privilege.php?act=add&role=gys', 'text' => '添加供应商账户'));
        $smarty->assign('full_page',   1);
        $smarty->assign('role',   'gys');
        $where = ' WHERE a.role_id=2';
        $smarty->assign('admin_list',  get_admin_userlist($where));
    }elseif($role == 'cxs'){
        $smarty->assign('ur_here',     '承销商账户列表');
        $smarty->assign('action_link', array('href'=>'privilege.php?act=add&role=cxs', 'text' => '添加承销商账户'));
        $smarty->assign('full_page',   1);
        $smarty->assign('role',   'cxs');
        $where = ' WHERE a.role_id=10 OR a.role_id=11';
        $smarty->assign('admin_list',  get_admin_userlist($where));
    }else{
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $role = isset($_GET['role']) ? trim($_GET['role']):'';
    if($role === 'gys'){
        $smarty->assign('ur_here',     '添加供应商账户');
        $smarty->assign('action_link', array('href'=>'privilege.php?act=list&role=gys', 'text' => '供应商账户列表'));
        $smarty->assign('selected_role','gys');
    }else{
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if($_GET['role']=='gys') {
        $link[0]['text'] = '返回列表';
        $link[0]['href'] = 'privilege.php?act=list&role=gys';
    }else{
        $link[0]['text'] = '返回列表';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if($role === 'gys'){
        ...
        $smarty->assign('action_link', array('text' => '供应商列表', 'href'=>'privilege.php?act=list&role=gys'));
    }else{
...
elseif ($_REQUEST['act'] == 'reset_password'){
    ...
    $newPass = 'xunluoyouxian';
    $password = md5(md5($newPass).$salt);
    $sql = "UPDATE ".$ecs->table('admin_user')." SET password='".$password."',ec_salt='".$salt."' WHERE user_id='".$id."'";
    $res = $db->query($sql);
    if(!$res){
        sys_msg('重置失败,请联系管理员');
    }else{
        sys_msg('密码已重置为 xunluoyouxian 请登录并修改密码');
    }
}
/*------------------------------------------------------ */
//-- 更新管理员信息
...
{
    if(in_array($_SESSION['role_id'], array(2,10))){
        sys_msg('不允许修改,如需修改请联系管理员', 0, $link);
    }
    /* 变量初始化 */
...
/* 获取管理员列表 */
function get_admin_userlist($where='')
{
    $list = array();
    $sql  = 'SELECT a.user_id, a.user_name, a.email, a.add_time, a.last_login, IFNULL(r.role_name,"未知") AS role_name '.
            'FROM ' .$GLOBALS['ecs']->table('admin_user').
            " AS a LEFT JOIN ".$GLOBALS['ecs']->table('role')." AS r ON a.role_id=r.role_id"
            .$where.' ORDER BY a.user_id DESC';
//    echo $sql;die;
