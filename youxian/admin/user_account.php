...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    $sql = "SELECT user_name,user_money,frozen_money,bank_account,bank_account_name,bank_name,bank_province,bank_city FROM " .$ecs->table('users'). " WHERE user_id = '$account[user_id]'";
    ...
    $smarty->assign('is_tixian', $account['process_type'] == 1?'1':'0');
    $smarty->assign('user_name',    $user_name);
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        {
            admin_priv('check_and_pay_user_tixian');
            ...
            if($fmt_amount<10){
                $link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
                sys_msg('提现金额不能小于10元', 0, $link);
            }
            $user_bank = $db->getRow("SELECT bank_account,bank_account_name,bank_name,bank_province,bank_city,bank_code FROM ".$ecs->table('users')." WHERE user_id='".$account['user_id']."'");
            //首信易代付
            require_once ROOT_PATH . 'includes/sxy.php';
            $sxy = new sxy();
            $format_id = 'U'.str_pad($id, 10, '0', STR_PAD_LEFT);
...
//            $df_data = "1|1.00|10776-20170928-0000000001$6210812450006318715|张晓欢|中国建设银行|河南省|洛阳市|1.00|20170925ceshi|105100000017";
            $df_data = "1|".$fmt_amount."|".$pc_sn."$".$user_bank['bank_account']."|".$user_bank['bank_account_name']."|".$user_bank['bank_name']."|".$user_bank['bank_province']."|".$user_bank['bank_city']."|".$fmt_amount."|".$format_id."|".$user_bank['bank_code'];
            ...
            if($daifu_res['message']['status']==0){
                ...
                $daifu_data = array(
                    'amount'=>$fmt_amount,
                    'sn'=>$pc_sn,
                    'card_no'=>$user_bank['bank_account'],
                    'card_user'=>$user_bank['bank_account_name'],
                    'bank'=>$user_bank['bank_name'],
                ...
                $res2 = update_user_account($id, $amount, $admin_note, $is_paid);
                //更新会员余额数量
                $res3 = log_account_change($account['user_id'], $amount, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);

                if($daifu_log && $res2 && $res3){
                    $db->query("COMMIT");
                }else{
                    $db->query("ROLLBACK");
                    $link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
                ...
                //代付失败
                echo '代付失败,请联系管理员.错误代码：'.$daifu_res['message']['statusdesc'];exit();
//                $link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
//                sys_msg($df_err_msg, 0, $link);
            }
            //首信易代付END
        }
