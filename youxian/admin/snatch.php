...
elseif ($_REQUEST['act'] =='insert')
{
    ...
    /* 检查商品是否存在 */
    $sql = "SELECT goods_name,goods_number FROM ".$ecs->table('goods')." WHERE goods_id = '$_POST[goods_id]'";
    $goodsa = $db->GetOne($sql);
    if (empty($goodsa["goods_name"]))
    {
        sys_msg($_LANG['no_goods'], 1);
        exit;
    }
    if ($goodsa["goods_number"]<$_POST["max_num"]){
        sys_msg($_LANG['num_over'], 1);
        exit;
    }
    $sql = "SELECT COUNT(*) ".
    ...
    $record = array('act_name'=>$_POST['snatch_name'], 'act_desc'=>$_POST['desc'],
                    ...
                    'short_price' =>$_POST['short_price'],'max_num' =>$_POST['max_num'],
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $finishied=local_date($GLOBALS['_CFG']['time_format'], gmtime());
    $smarty->assign('finished',     $finishied);
...
function get_snatchlist()
{
    ...
    if ($result === false)
    {
        ...
        /* 获活动数据 */
        $sql = "SELECT act_id, act_name AS snatch_name, goods_name, start_time, end_time, is_finished, max_num, short_price, ext_info, product_id ".
...
function get_snatch_info($id)
{
    ...
    $sql = "SELECT act_id, act_name AS snatch_name,max_num, short_price, goods_id, product_id, goods_name, start_time, end_time, act_desc, ext_info" .
           " FROM " . $GLOBALS['ecs']->table('goods_activity') .
...
function get_snatch_detail()
{
    ...
    /* 获得活动数据 */
    $sql = "SELECT s.log_id, u.user_name, u.email, s.bid_price, s.bid_time ".
