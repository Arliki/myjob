...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        ...
        admin_log('类型ID:'.$attr['cat_id'].'属性名称:'.$_POST['attr_name'], 'add', 'attribute');
        $links = array(
    ...
    else
    {
        ...
        admin_log('类型ID:'.$attr['cat_id'].'属性名称:'.$_POST['attr_name'], 'edit', 'attribute');
        $links = array(
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        /* 记录日志 */
        admin_log(''.substr(db_create_in($ids),3), 'batch_remove', 'attribute');
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $id = intval($_GET['id']);
    $act_text = '属性id'.$id;
    $cat_id = $db->getOne("SELECT cat_id FROM ".$ecs->table('attribute')." WHERE attr_id='$id'");
    if($cat_id){
        $act_text .= ',类型id'.$cat_id;
    }
    $db->query("DELETE FROM " .$ecs->table('attribute'). " WHERE attr_id='$id'");
    $db->query("DELETE FROM " .$ecs->table('goods_attr'). " WHERE attr_id='$id'");

    admin_log($act_text, 'remove', 'attribute');
