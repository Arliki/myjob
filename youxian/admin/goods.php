...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $condition = $_SESSION['role_id']==2 ? ' AND add_user_id='.$_SESSION['admin_id'] : '';
    $goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1, $condition);
...
//var_dump($_SESSION);
    ...
    else
    {
        ...
        else
        {
         ...
         $goods['sell_start_date'] = local_date('Y-m-d', $goods['sell_start_date']);
            $goods['sell_end_date'] = local_date('Y-m-d', $goods['sell_end_date']);
        /* 如果是复制商品，处理 */
    ...
    $smarty->assign('gys_list', gys_list_name());
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    //半价
    $half_price = isset($_POST['half_price']) ? intval($_POST['half_price']) : '0';
    ...
    //自定义销量
    $sale_count = empty($_POST['sale_count']) ? 0 : intval($_POST['sale_count']);
    //拼单属性
    $collage_size = isset($_POST['collage_size']) ? $_POST['collage_size'] : 0;
    $collage_max = isset($_POST['collage_max']) ? $_POST['collage_max'] : 0;
    $collage_time = isset($_POST['collage_time']) ? $_POST['collage_time'] : 0;
    $collage_f_p = isset($_POST['collage_f_p']) ? $_POST['collage_f_p'] : 0;
    $collage_o_p = isset($_POST['collage_o_p']) ? $_POST['collage_o_p'] : 0;
    $collage_time>72?$collage_time=72:'';
    $collage_time<6?$collage_time=6:'';
	...
	$sell_start_date =( !empty($_POST['sell_start_date'])) ? local_strtotime($_POST['sell_start_date']) : 0;
	$sell_end_date = ( !empty($_POST['sell_end_date'])) ? local_strtotime($_POST['sell_end_date']) : 0;
	$add_user_id = intval($_SESSION['admin_id']);
    $gys_id = isset($_POST['gys_id']) ? intval($_POST['gys_id']) : '0';
    /* 入库 */
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date,sell_start_date, sell_end_date,  goods_img, goods_thumb, original_img, keywords, goods_brief, sale_count, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, add_user_id, gys_id,collage_max,collage_size,collage_time,collage_f_p,collage_o_p,half_price)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date','$sell_start_date', '$sell_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$sale_count', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$add_user_id', '$gys_id','$collage_max','$collage_size','$collage_time','$collage_f_p','$collage_o_p','$half_price')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, add_user_id, gys_id,collage_max,collage_size,collage_time,collage_f_p,collage_o_p,half_price)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date','$sell_start_date', '$sell_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$sale_count', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$add_user_id', '$gys_id','$collage_max','$collage_size','$collage_time','$collage_f_p','$collage_o_p','$half_price')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
              ...
              "sell_end_date = '$sell_end_date', " .
                ...
                "collage_max='$collage_max',".
                "collage_size='$collage_size',".
                "collage_time='$collage_time',".
                "collage_f_p='$collage_f_p',".
                "collage_o_p='$collage_o_p',".
    ...
    if ($is_insert)
    {
        admin_log($_POST['goods_name'].'[供应商ID:'.$gys_id.']', 'add', 'goods');
    }
    else
    {
        admin_log($_POST['goods_name'].'[供应商ID:'.$gys_id.']', 'edit', 'goods');
    }
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
                ...
                $attr_gys_price = $_POST['attr_gys_price_list'][$key];
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['attr_gys_price'] = $attr_gys_price;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price, attr_gys_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]', '$info[attr_gys_price]')";

                    $act_label = 'add';
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]', attr_gys_price = '$info[attr_gys_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                ...
                $db->query($sql);
                $act_text = '商品id'.$goods_id.'属性id'.$attr_id.',供应商价格：'.$info['attr_gys_price'];
                admin_log($act_text, $act_label, 'attribute');
            }
        }
    }
...
//-- 修改自定义库存
elseif ($_REQUEST['act'] == 'edit_goods_sale_count'){
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $sale_count       = intval($_POST['val']);

    if ($exc->edit("sale_count = '$sale_count'", $goods_id))
    {
        clear_cache_files();
        make_json_result($sale_count);
    }
}
/*------------------------------------------------------ */
//-- 修改商品库存数量
