...
if ($_REQUEST['act'] == 'list')
{
    ...
    foreach($user_list['user_list'] as $k=>$v){
        if($v['user_rank'] == 6){
            $v['jjr_name'] = $v['user_name'];
        }
        $user_list['user_list'][$k] = $v;
    }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    u.office_phone, u.home_phone, u.mobile_phone, u.dls_id".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
...
//        $user['formated_frozen_money'] = price_format(0);
    ...
    $dls_list = $db->getAll("SELECT * FROM ".$ecs->table('dls')." WHERE 1");
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $other['home_phone'] = isset($_POST['extend_field4']) ? htmlspecialchars(trim($_POST['extend_field4'])) : '';
    $other['mobile_phone'] = isset($_POST['extend_field5']) ? htmlspecialchars(trim($_POST['extend_field5'])) : '';
    $other['dls_id'] = isset($_POST['dls_id']) ? intval(trim($_POST['dls_id'])) : 0;
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,user_rank,(SELECT username FROM ".$GLOBALS['ecs']->table('dls')." WHERE dls_id=u.dls_id) AS dls_name,(SELECT user_name FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id=u.jjr_id) AS jjr_name  ".
