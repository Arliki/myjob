...
//-- 订单查询
/*------------------------------------------------------ */
//if($_REQUEST['act'] == 'fill_order_goods_gys_price'){
//    return;
//    admin_priv('');
//    $order_goods = $db->getAll("SELECT * FROM ".$ecs->table('order_goods')." where gys_price=0");
//
//    foreach($order_goods as $k=>$v){
//        $v['gys_price'] = get_goods_gys_price($v['goods_id'], $v['goods_attr_id']);
//        $db->query("UPDATE ".$ecs->table('order_goods')." SET gys_price='".$v['gys_price']."' WHERE rec_id='".$v['rec_id']."'");
//        echo $v['rec_id']."<br/>";
//    }
//
//}
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $gys_order_desc = get_gys_count_desc();
    $smarty->assign('gys_order_desc', $gys_order_desc);
    if(!isGys() && check_authz('order_view')){
        $smarty->assign('showTotalFee', true);
    }
    $can_export_order = admin_priv('order_export','',false);
    $smarty->assign('can_export_order',$can_export_order);
    /* 显示模板 */
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $smarty->assign('agency_list', $db->getAll($sql));
    /* 是否有指派供应商的权限 */
    $can_assign_gys = admin_priv('assign_order_agency','',false);
    $smarty->assign('can_assign_gys', $can_assign_gys);
    /* 取得区域名 */
...
//    echo $order['gys_price'];die;
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        /*如果是供应商,处理属性字段，隐藏商城销售价*/
        if(isGys()){
            $tmpAttr = $row['goods_attr'];
            if(preg_match('/\[/', $tmpAttr)){
                $row['goods_attr'] = preg_replace('/\[[\d\.]+\]/', '', $tmpAttr);
            }
        }
        /*END*/
        ...
        $goods_gys_price = get_goods_gys_price($row['goods_id'], $row['goods_attr_id']);
        $row['goods_gys_price'] = price_format($goods_gys_price);
        $row['goods_gys_total'] = price_format($goods_gys_price * $row['goods_number']);
        $goods_list[] = $row;
    }
...
//    $isAdmin = isXlAdmin() || isSuperAdmin();
    $canCheckShippingNo = check_authz('check_gys_shipping_and_pay_first');
    $smarty->assign('canCheckShippingNo', $canCheckShippingNo);
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    //供应商取消配货和修改订单的权限检查
    if(isGys()){
        if($step != 'shipping' and $step != 'money'){
...
//            }
    ...
    elseif ('shipping' == $step)
    {
        ...
        if($new_order['order_amount']=='0'){
            $sql = "UPDATE ".$ecs->table('order_info')." SET pay_status='2' WHERE order_id='".$new_order['order_id']."'";
            $db->query($sql);
        }
    ...
    elseif ('money' == $step)
    {
        ...
        $order['bonus_id']      = 0;
        $order['bonus']         = 0;
        $order['coupon_money']  = $old_order['coupon_money'];
        /* 计算待付款金额 */
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    {
        //手动指派订单给供应商
        admin_priv('assign_order_agency');
        /* 取得参数 */
...
//        }
    ...
    {
        admin_log('尝试删除订单：'.$order['order_sn'], 'remove', 'order');
        sys_msg('暂时不允许删除');
        $require_note = false;
        $operation = 'remove';
        if (!$batch)
        {
            ...
            $hasGysInAccount = $db->getOne("SELECT COUNT(*) FROM ".$ecs->table('gys_earn_log')." WHERE order_id='$order_id'");
            if($hasGysInAccount>0){
                sys_msg('该订单有供应商分成记录,不允许删除');
            }
    ...
    elseif (isset($_POST['check_gys_shipping_and_pay_first']))
    {
        $require_note   = false;
        $action         = '单号无误,结算供应商首期款';
        $operation      = 'check_gys_shipping_and_pay_first';
    }
    /* 直接处理还是跳到详细页面 */
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('unpay' == $operation)
    {
        ...
        /*处理订单供应商出账*/
        $res = gys_out_account($order_id, '管理员修改订单付款状态为未付款,取消已入账收益');
        if(is_array($res) && $res['error']==1){
            admin_log($res['info'],'修改订单'.$order_id,'付款状态为未付款');
        }
        /*订单供应商出账END*/
    }
    /* 配货 */
...
//            $shipping_status = SS_SHIPPED_ING;
    ...
    elseif ('cancel' == $operation)
    {
        ...
        /*处理订单供应商出账*/
        $res = gys_out_account($order_id, '管理员取消订单,取消已入账收益');
        if(is_array($res) && $res['error']==1){
            admin_log($res['info'],'取消订单'.$order_id,'');
        }
        /*订单供应商出账END*/
    ...
    elseif ('invalid' == $operation)
    {
        ...
        /*处理订单供应商出账*/
        $res = gys_out_account($order_id, '管理员设置订单为无效,取消已入账收益');
        if(is_array($res) && $res['error']==1){
            admin_log($res['info'],'修改订单'.$order_id,'无效订单');
        }
        /*订单供应商出账END*/
    ...
    elseif ('return' == $operation)
    {
        ...
        /*处理订单供应商出账*/
        $res = gys_out_account($order_id, '管理员退货,取消已入账收益');
        if(is_array($res) && $res['error']==1){
            admin_log($res['info'],'退货'.$order_id,'');
        }
        /*订单供应商出账END*/
    ...
    /*审核物流单号，并结算供应商首期款*/
    elseif ('check_gys_shipping_and_pay_first' == $operation){
        admin_priv('check_gys_shipping_and_pay_first');
        $res = check_gys_shipping_and_pay_first($order_id);
        if($res['error']==1){
            sys_msg($res['info']);
        }
    }
...
elseif ($_REQUEST['act'] == 'export_order'){
    admin_priv('order_export');
    ...
    $composite_status=$_GET['composite_status'];
    $sql = "SELECT o.order_id,o.order_sn,o.add_time,o.country,o.province,o.city,o.district,o.address,o.consignee,o.mobile,o.order_status,o.pay_status,o.shipping_status,a.agency_name,og.goods_name,og.goods_sn,og.goods_price,og.goods_number,og.goods_attr,(" . order_amount_field('o.') . ") AS total_fee, o.order_amount FROM ".$ecs->table('order_info')." AS o LEFT JOIN ".$ecs->table('order_goods')." AS og ON og.order_id=o.order_id LEFT JOIN ".$ecs->table('agency')." AS a ON a.agency_id=o.agency_id where 1 ";
    if ($filter['order_sn'])
    {
        $where .= " AND o.order_sn LIKE '%" . mysql_like_quote($order_sn) . "%'";
    }
    if ($filter['consignee'])
    {
        $where .= " AND o.consignee LIKE '%" . mysql_like_quote($consignee) . "%'";
    }
    ...
    $where .=" and o.add_time>'".$btime."' and o.add_time < '".$etime."' ORDER BY o.order_id desc,og.goods_name DESC";
    $sql=$sql.$where;
    /* csv文件数组 */
    $orders = array();
    $orders['order_id'] = '""';
    ...
    $orders['goods_name'] = '""';
    $orders['goods_sn'] = '""';
    $orders['goods_price'] = 0.00;
    $orders['goods_number'] = 0;
    $orders['goods_attr'] = '""';
    $orders['agency_name'] = '""';
    $orders['gys_price'] = 0.00;
    $orders['total_fee'] = 0.00;
    $orders['order_amount'] = 0.00;
    $orders['outadd'] = '""';
    $theader = array('编号','订单号','下单时间','收货人','商品名称','货号','价格','数量','属性','供应商','供货价','总金额','应付金额','收货地址','订单状态');
    $content = '"' . implode('","', $theader) . "\"\n";
    $row = $db->getAll($sql);
    $nums=1;
//    导出数据
    for ($k=0;$k<1001;$k+=$nums){
        if($row[$k]['order_id']) {
            $order_id = $row[$k]['order_id'];
            $order_sn = $row[$k]['order_sn'];
            $add_time = local_date('Y-m-d H:i:s', $row[$k]['add_time']);
            $consignee = $row[$k]['consignee'].'['.$row[$k]['mobile'].']';


            $goods_name = $row[$k]['goods_name'];
            $goods_sn = $row[$k]['goods_sn'];
            $goods_price = $row[$k]['goods_price'];
            $goods_number = $row[$k]['goods_number'];
            $goods_attr = $row[$k]['goods_attr'];

            $nums = 1;
            for ($i = 1; $i < 100; $i++) {
                if ($row[$k + $i]['order_sn'] == $row[$k + $i - 1]['order_sn']) {
                    $goods_name .= "\n" . $row[$k + $i]['goods_name'];
                    $goods_sn .= "\n" . $row[$k + $i]['goods_sn'];
                    $goods_price .= "\n" . $row[$k + $i]['goods_price'];
                    $goods_number .= "\n" . $row[$k + $i]['goods_number'];
                    $goods_attr .= "\n" . $row[$k + $i]['goods_attr'];
                    $nums += 1;
                } else {
                    $nums=1;
            ...
            $agency_name = $row[$k]['agency_name'];
            $gys_price = get_order_gys_price($row[$k]['order_id']);
            $total_fee = $row[$k]['total_fee'];
            $order_amount = $row[$k]['order_amount'];

            //添加地址
            $put_addre=$db->getAll("select a.region_name c1,b.region_name c2,c.region_name c3,d.region_name c4 from ecs_region as a left join ecs_region as b on b.parent_id=a.region_id left join ecs_region as c on c.parent_id=b.region_id left join ecs_region as d on d.parent_id=c.region_id where a.region_id='".$row[$k]['country']."' and b.region_id='".$row[$k]['province']."' and c.region_id='".$row[$k]['city']."' and d.region_id='".$row[$k]['district']."' ");
            $outadd="[".$put_addre[0]["c1"]."--".$put_addre[0]["c2"]."--".$put_addre[0]["c3"]."--".$put_addre[0]["c4"]."] ".$row[$k]['address'];


            $order_status = preg_replace('[<font color="red">|<\/font>]', '', $_LANG['os'][$row[$k]['order_status']]) . ',' . $_LANG['ps'][$row[$k]['pay_status']] . ',' . $_LANG['ss'][$row[$k]['shipping_status']];

            $orders['order_id'] = '"' . $order_id . '"';
            $orders['order_sn'] = '"[' . $order_sn . ']"';
            $orders['add_time'] = '"' . $add_time . '"';
            $orders['consignee'] = '"' . $consignee . '"';

            $orders['goods_name'] = '"' . $goods_name . '"';
            $orders['goods_sn'] = '"' . $goods_sn . '"';
            $orders['goods_price'] = '"' . $goods_price . '"';
            $orders['goods_number'] = '"' . $goods_number . '"';
            $orders['goods_attr'] = '"' . $goods_attr . '"';
            $orders['agency_name'] = '"'.$agency_name.'"';
            $orders['gys_price'] = '"' . $gys_price . '"';

            $orders['total_fee'] = '"' . $total_fee . '"';
            $orders['order_amount'] = '"' . $order_amount . '"';

            $orders['outadd'] = '"' . $outadd . '"';//添加地址

            $orders['order_status'] = '"' . $order_status . '"';
            $content .= implode(",", $orders) . "\n";
        }
    }
    $charset = 'gb2312';
    $export_file_name = date('Ymd',time()).'订单列表';
...
function operable_list($order)
{
    ...
    /*审核快递单并结算首期款*/
    if($ss == SS_SHIPPED && $ps == PS_PAYED){
        $list['check_gys_shipping_and_pay_first'] = true;
    }
    return $list;
}
 ...
 * @return array
 */
function get_gys_count_desc()
{
    $sql = "SELECT order_id FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE pay_status='2'";
    if(isGys()){
        $agency_id = $GLOBALS['db']->getOne("SELECT agency_id FROM ".$GLOBALS['ecs']->table('admin_user')." WHERE user_id='$_SESSION[admin_id]'");
        $sql .= " AND agency_id = '$agency_id'";
    }
    $today_sql = $sql." AND add_time>".strtotime(date('Y-m-d', time()));
    $all_order_ids = $GLOBALS['db']->getAll($sql);
    $today_order_ids = $GLOBALS['db']->getAll($today_sql);
    $total_gys_order_amount = 0.00;
    $today_gys_order_amount = 0.00;
    foreach ($all_order_ids as $order){
        $total_gys_order_amount += get_order_gys_price($order['order_id']);
    }
    foreach ($today_order_ids as $torder){
        $today_gys_order_amount += get_order_gys_price($torder['order_id']);
    }
    return array(
        'total_amount'=>price_format($total_gys_order_amount),
        'today_amount'=>price_format($today_gys_order_amount)
    );
}
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['agency_id'] = empty($_REQUEST['agency_id']) ? 0 : intval($_REQUEST['agency_id']);

        $where = 'WHERE 1 ';

        if($filter['agency_id']){
            $where .= 'AND o.agency_id='.$filter['agency_id'];
        }
        ...
        if(isGys()){
            if(!$_SESSION['agency_id']){
                sys_msg('供应商账号异常,没有绑定供应商信息');
            }
            $where .= " AND o.agency_id = '".$_SESSION['agency_id']."'";
        }
...
//        }
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
            "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, a.agency_name, " .
            "(" . order_amount_field('o.') . ") AS total_fee, " .
            "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ".
            " LEFT JOIN ".$GLOBALS['ecs']->table('agency'). " AS a ON o.agency_id=a.agency_id ".
            $where .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $order_gys_price = get_order_gys_price($value['order_id']);
        $row[$key]['order_gys_price'] = '￥'.$order_gys_price.'元';
        $totalGysPrice += $order_gys_price;
    }
    $GLOBALS['_LANG']['putt']=$row;
    $arr = array('orders' => $row,'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count'],'totalGysPrice'=>$totalGysPrice);
//var_dump($arr);
    return $arr;
}
