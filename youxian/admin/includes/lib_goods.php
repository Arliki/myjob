...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price, v.attr_gys_price ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            ...
            '  供货商供应价<input type="text" name="attr_gys_price_list[]" value="' . $val['attr_gys_price'] . '" size="5" maxlength="10" />' :
            ' <input type="hidden" name="attr_price_list[]" value="0" /><input type="hidden" name="attr_gys_price_list[]" value="0" />';
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, cat_id, goods_name, goods_type, goods_sn, shop_price, sale_count, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
