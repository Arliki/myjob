 ...
 * @param   integer $role_id        最后登录时间
 * @return  void
 */
function set_admin_session($user_id, $username, $action_list, $last_time, $role_id)
{
    $_SESSION['admin_id']    = $user_id;
    $_SESSION['admin_name']  = $username;
    $_SESSION['action_list'] = $action_list;
    $_SESSION['last_check']  = $last_time; // 用于保存最后一次检查订单的时间
    $_SESSION['role_id']  = $role_id; // 管理员角色
}
...
function gys_list_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
...
function gys_list_name()
{
    /* 查询 */
    $gys_list = gys_list_info();

    /* 供货商名字 */
    $gys_name = array();
    if (count($gys_list) > 0)
    {
        foreach ($gys_list as $gys)
        {
            $gys_name[$gys['agency_id']] = $gys['agency_name'];
        }
    }
...
function isCxs(){
    return $_SESSION['role_id'] == 10 || $_SESSION['role_id'] == 11;
}
?>