 ...
 * ECSHOP ?у??伴?椤
 * ============================================================================
 * * ?????? 2005-2012 涓?捣??淳缃??绉????????锛?苟淇????????┿?
 * 缃???板?: http://www.ecshop.com锛
 * ----------------------------------------------------------------------------
 * 杩?????涓???辫蒋浠讹??ㄥ??藉?涓??浜??涓????????涓??绋??浠ｇ?杩??淇???
 * 浣跨?锛?????瀵圭?搴?唬??互浠讳?褰㈠?浠讳???????????
 * ============================================================================
 * $Author: liubo $
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    {
        unset($_SESSION['shop_guide']);//??姣?ession
    ...
    {
        /* 濡?? open_basedir 涓?负绌猴?????ユ??????? upload_tmp_dir  */
...
elseif ($_REQUEST['act'] == 'third')
{
    ...
    if(!empty($good_category))
    {
        ...
        if(!empty($good_name))
        {
            /* 妫??ュ????濡?????璇??妫??ュ昂瀵告????杩??澶у?硷????锛???ユ?浠剁被? */
            if (isset($_FILES['goods_img']['error'])) // php 4.2 ??????? error
            {
                // ??澶т?浼??浠跺ぇ灏
            ...
            if ($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none')
            {
                ...
                $original_img   = $image->upload_image($_FILES['goods_img']); // ????剧?
                if ($original_img === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
                $goods_img      = $original_img;   // ????剧?

                /* 澶??涓?浠界????? */
                $img        = $original_img;   // ?稿??剧?
    ...
    assign_query_info();
    //    $smarty->assign('ur_here', '寮?搴??瀵硷?娣诲????');
...
elseif ($_REQUEST['act'] == 'check_order')
{
    ...
    //如果是供应商,添加检索条件
    $gys_id = $db->getOne("SELECT agency_id FROM ".$ecs->table('admin_user')." WHERE user_id=".$_SESSION['admin_id']);
    $where = isGys() ? ' AND agency_id='.$gys_id : '';

    /* ?拌?? */
    $sql = 'SELECT COUNT(*) FROM ' . $ecs->table('order_info').
    " WHERE add_time >= '$_SESSION[last_check]'".$where;
    $arr['new_orders'] = $db->getOne($sql);

    /* ?颁?娆剧?璁㈠? */
    $sql = 'SELECT COUNT(*) FROM '.$ecs->table('order_info').
    ' WHERE pay_time >= ' . $_SESSION['last_check'].$where;
...
elseif ($_REQUEST['act'] == 'send_mail')
{
    ...
    //濡????ā?匡????宸插???mail_sendlist???瀹逛?涓洪?浠跺?瀹
    //????虫???川锛??mail_templates璋?????瀹逛?涓洪?浠跺?瀹
    if ($rt['type'] == 'template')
    {
