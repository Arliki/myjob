...
if ($_REQUEST['act'] == 'add')
{
    ...
    $clas="select cat_id,cat_name from ecs_category where is_exchange=1";
    $clas = $db->GetAll($clas);
    $smarty->assign('clas',       $clas);
    $smarty->assign('is_add',       1);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $cat_id=empty($_POST['cat_id'])?191:$_POST['cat_id'];
    $sql = "INSERT INTO ".$ecs->table('exchange_goods')."(goods_id, exchange_integral, is_exchange, is_hot ,cat_id) ".
            "VALUES ('$_POST[goods_id]', '$_POST[exchange_integral]', '$_POST[is_exchange]', '$_POST[is_hot]' ,'$cat_id')";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取商品数据 */
    $sql = "SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange, eg.is_hot, g.goods_name ,eg.cat_id ".
...
if ($_REQUEST['act'] =='update')
{
    ...
    if ($exc->edit("exchange_integral='$_POST[exchange_integral]', is_exchange='$_POST[is_exchange]', is_hot='$_POST[is_hot]' ,cat_id='$_POST[cat_id]'", $_POST['goods_id']))
    {
...
function get_exchange_goodslist()
{
    ...
    if ($result === false)
    {
        ...
        /* 获取文章数据 */
        $sql = 'SELECT eg.* , g.goods_name ,c.cat_name ,c.cat_id '.
               'FROM ' .$GLOBALS['ecs']->table('exchange_goods'). ' AS eg '.
               'LEFT JOIN ' .$GLOBALS['ecs']->table('goods'). ' AS g ON g.goods_id = eg.goods_id '.
                'LEFT JOIN '.$GLOBALS['ecs']->table('category').'AS c ON c.cat_id=eg.cat_id '.
